...
/**
 * CTestStateData is a specialized HashMap implementation designed for managing test state data.
 * It extends CHashMap to provide type-safe storage and retrieval of test-related data using
 * string keys and generic object values.
 * 
...
public class CTestStateData extends CHashMap<String, Object> {
  /**
   * Constructs an empty CTestStateData with the default initial capacity (16)
  ...
  /**
   * Constructs an empty CTestStateData with the specified initial capacity and the default load factor (0.75).
   * 
   * @param initialCapacity the initial capacity of the hash map
  ...
  /**
   * Constructs a new CTestStateData with the same mappings as the specified Map.
   * The CTestStateData is created with default load factor (0.75) and an initial capacity
  ...
  /**
   * Updates the test state data by associating the specified value with the specified key.
   * If the map previously contained a mapping for the key, the old value is replaced.
   * 
   * @param key the key with which the specified value is to be associated
   * @param value the value to be associated with the specified key
   * 
   * <p>This method is a convenience wrapper around the standard {@code put} method,
   * providing a more descriptive name for test state management contexts.
   * 
   * <p>Example usage:
   * <pre>{@code
   * CTestStateData testData = new CTestStateData();
  ...
  /**
   * Retrieves the test state data associated with the specified key and casts it to the expected type.
   * Returns {@code null} if this map contains no mapping for the key.
   * 
   * @param <T> the expected type of the returned value
   * @param key the key whose associated value is to be returned
   * @return the value to which the specified key is mapped, cast to type T, or {@code null} if this map contains no mapping for the key
   * @throws ClassCastException if the value cannot be cast to the expected type T
   * 
   * <p>This method provides type-safe retrieval of stored values. The caller is responsible
   * for ensuring the correct type is specified to avoid ClassCastException.
   * 
   * <p>Example usage:
   * <pre>{@code
   * CTestStateData testData = new CTestStateData();
