...
/**
 * Custom retry analyzer for TestNG tests that extends the built-in RetryAnalyzerCount.
 * This class provides functionality to automatically retry failed tests based on a
 * configurable retry count obtained from CTestNGConfigs.
 * 
 * <p>The retry analyzer allows tests to be re-executed when they fail, which is useful
 * for handling flaky tests or transient failures in test environments.</p>
 * 
 * <h3>Usage Example:</h3>
 * <pre>{@code
...
public class CRetryAnalyzer extends RetryAnalyzerCount {
  /**
   * Constructs a new CRetryAnalyzer and initializes the retry count.
   * The retry count is obtained from the configuration and set automatically.
   * 
   * <h3>Example:</h3>
   * <pre>{@code
  ...
  /**
   * Resets the retry count to the value configured in CTestNGConfigs.
   * This method fetches the retry count from the configuration and sets it
   * as the maximum number of retries allowed for this analyzer.
   * 
   * <h3>Example:</h3>
   * <pre>{@code
   * CRetryAnalyzer analyzer = new CRetryAnalyzer();
  ...
  /**
   * Checks if there are any retry executions left for the current test.
   * Returns true if the retry count is greater than -1, indicating that
  ...
  /**
   * Determines if the current execution is the last retry attempt.
   * Returns true when the retry count is less than 1, indicating that
   ...
   * This implementation always returns true, meaning all failed tests
   * will be retried up to the configured retry count limit.
   * 
   * <p>This method is called by TestNG framework when a test fails.
   * The actual retry logic and count management is handled by the
   * parent RetryAnalyzerCount class.</p>
   * 
   * <h3>Example:</h3>
   * <pre>{@code
