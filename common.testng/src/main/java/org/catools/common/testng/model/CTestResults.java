...
/**
 * A specialized collection that extends {@link CList} to manage a list of {@link CTestResult}
 * objects. This class provides convenient methods for querying, filtering, and organizing test
 * results from TestNG test executions.
 *
 * <p>The class offers functionality to:
 *
 * <ul>
 *   <li>Find test results by TestNG method or test ID
 *   <li>Check test execution status
 *   <li>Collect all issue IDs from test results
 *   <li>Group test results by execution status
 * </ul>
 *
 * @see CTestResult
...
public class CTestResults extends CList<CTestResult> {
   ...
   *
   * <p>This method searches for a test result that matches both the class name and method name of
   * the provided TestNG method.
   *
   ...
   *
   * <p>This method searches for a test result matching the provided method and checks if its status
   * indicates a passed state.
   *
   ...
   *
   * <p>This method searches for a test result that contains the specified ID in its test IDs
   * collection. Test IDs are typically used to link test cases to external test management systems
   * or issue trackers.
   *
   ...
   *
   * <p>This method aggregates test IDs from all test results, which can be useful for reporting,
   * tracking coverage, or integration with external systems like JIRA or other test management
   * tools.
   *
   ...
   *
   * @return a {@link CHashMap} where keys are {@link CExecutionStatus} values and values are {@link
   *     CTestResults} collections containing tests with that status
   *     <p>Example:
   *     <pre>
