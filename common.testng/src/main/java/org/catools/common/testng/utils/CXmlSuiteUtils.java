 ...
 *
 * <p>This utility class provides methods to dynamically build TestNG XML test suites and tests from
 * various sources such as issue keys, test class names, and existing tests. It integrates with the
 * catools TestNG configuration system to apply parallel execution settings and other test execution
 * parameters.
 *
 * <p>The class supports:
 *
 * <ul>
 *   <li>Building tests from issue keys with optional filtering
 *   <li>Creating test suites from collections of test classes
 *   <li>Configuring parallel execution at suite and test levels
 *   <li>Copying and modifying existing test suites
 * </ul>
 *
 * <p>Example usage:
 *
...
public class CXmlSuiteUtils {
   ...
   *
   * <p>This method converts issue keys into their corresponding test class names and creates an XML
   * test configuration. The issue keys are resolved to test classes using the {@link
   * CTestClassUtil#getClassNameForIssueKeys} method.
   *
   ...
   * @param filterTestsWhichWillSkipInRun if true, excludes test classes that would be skipped
   *     during execution due to configuration filters (annotations, severity, etc.)
   ...
   */
  public static XmlTest buildTestForIssueKeys(
      CSet<String> issueIds, String testName, boolean filterTestsWhichWillSkipInRun) {
    CSet<String> classNameForIssueKeys =
   ...
   *
   * <p>This method creates a complete test suite containing multiple tests, where each test is
   * built from a collection of test classes. The method automatically applies parallel execution
   * configuration from {@link CTestNGConfigs} and allows for custom suite-level adjustments through
   * the xmlSuiteAdjuster parameter.
   *
   ...
   * @param xmlSuiteAdjuster optional consumer function to perform additional configuration on the
   *     suite after it's created (can be null)
  ...
  public static XmlSuite buildTestSuiteForClasses(
    ...
    CList<XmlTest> tests =
        testClasses.asSet().mapToList(e -> buildTestForClasses(e.getValue(), e.getKey()));
   ...
   *
   * <p>This method creates an XmlTest object configured with the specified test classes. The test
   * classes are automatically sorted alphabetically to ensure consistent test execution order
   ...
   *
   * <p>This method creates a fully configured TestNG test suite with the following features:
   *
   * <ul>
   *   <li>Configures parallel execution settings from {@link CTestNGConfigs}
   *   <li>Sets up thread counts for both suite and test levels
   *   <li>Enables return values (required for some TestNG features)
   *   <li>Applies custom configuration through the xmlSuiteAdjuster
   *   <li>Properly associates all tests with the suite
   * </ul>
   *
   * <p>Parallel execution is configured at two levels:
   *
   * <ul>
   *   <li><strong>Suite Level:</strong> Controls how multiple tests within the suite run in
   *       parallel
   *   <li><strong>Test Level:</strong> Controls how test methods within each test run in parallel
   * </ul>
   *
   * @param tests a list of XmlTest objects to include in the suite
   * @param suiteName the name to assign to the created XML test suite
   * @param xmlSuiteAdjuster optional consumer function to perform additional configuration on the
   *     suite after basic setup is complete (can be null)
   ...
   */
  public static XmlSuite buildTestSuiteForTests(
      CList<XmlTest> tests, String suiteName, Consumer<XmlSuite> xmlSuiteAdjuster) {
   ...
   *
   * <p>This method performs a deep clone of the provided XmlSuite object and modifies the suite
   * name by appending the specified postfix. This is useful for creating variations of existing
   * test suites (e.g., for different environments, retry runs, or parallel execution scenarios).
   *
   * <p>The copy includes all configuration from the original suite:
   *
   * <ul>
   *   <li>All tests and their associated test classes
   *   <li>Parallel execution settings
   *   <li>Thread counts and other execution parameters
   *   <li>Suite-level parameters and listeners
   *   <li>All other suite-level configuration
   * </ul>
   *
