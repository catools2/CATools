...
/**
 * Enumeration representing different execution statuses for test cases or test runs.
 * Each status has an associated priority ID that indicates its importance level.
 * 
 * <p>This enum provides utility methods to categorize statuses into logical groups
 * such as running, failed, skipped, etc., which helps in test result analysis
 * and reporting.</p>
 * 
...
public enum CExecutionStatus {
   ...
   * 
   * <p>A test is considered running if it has the status {@link #WIP} (Work In Progress).</p>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   * CExecutionStatus status = CExecutionStatus.WIP;
   ...
   * 
   * <p>A test is considered failed if it has the status {@link #FAILURE}.</p>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   ...
   * 
   * <p>A test is considered skipped if it has one of the following statuses:</p>
   * <ul>
   *   <li>{@link #SKIP} - Test was explicitly skipped during execution</li>
   *   <li>{@link #IGNORED} - Test was marked to be ignored</li>
   *   <li>{@link #DEFERRED} - Test execution was postponed</li>
   * </ul>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   ...
   * }</pre>
   * 
   * @return {@code true} if the status is one of {@link #SKIP}, {@link #IGNORED}, 
   *         or {@link #DEFERRED}, {@code false} otherwise
   */
   ...
   * 
   * <p>A test is considered awaiting if it has the status {@link #AWAITING}, indicating
   * it is queued for execution but has not yet started.</p>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   ...
   * 
   * <p>A test is considered blocked if it has the status {@link #BLOCKED}, indicating
   * that external dependencies or conditions prevent the test from executing.</p>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   ...
   * 
   * <p>A test is considered passed if it has one of the following statuses:</p>
   * <ul>
   *   <li>{@link #SUCCESS} - Test completed successfully with all assertions passing</li>
   *   <li>{@link #SUCCESS_PERCENTAGE_FAILURE} - Test passed overall but with some 
   *       percentage of sub-component failures</li>
   * </ul>
   * 
   * <h3>Usage Example:</h3>
   * <pre>{@code
   ...
   * }</pre>
   * 
   * @return {@code true} if the status is {@link #SUCCESS} or 
   *         {@link #SUCCESS_PERCENTAGE_FAILURE}, {@code false} otherwise
   */
