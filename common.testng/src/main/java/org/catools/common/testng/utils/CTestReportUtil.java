 ...
 *
 * <p>This class provides functionality to handle duplicate test results that can occur when running
 * TestNG suites multiple times or in parallel execution scenarios. It ensures that test results are
 * properly deduplicated while maintaining the correct status hierarchy (passed > failed > skipped).
 *
 * @author CATools Team
 * @since 1.0
 */
...
public class CTestReportUtil {
  ...
  /**
   * Removes duplicate test results from TestNG suites while maintaining proper test status
   * hierarchy.
   *
   * <p>This method processes a list of TestNG suites and eliminates duplicate test results that may
   * occur when the same test method is executed multiple times. The deduplication follows a strict
   * hierarchy where:
   *
   * <ul>
   *   <li>Passed tests take precedence over failed and skipped tests
   *   <li>Failed tests take precedence over skipped tests
   *   <li>Configuration methods follow the same hierarchy as test methods
   * </ul>
   *
   * <p>The method is thread-safe and performs the following operations:
   *
   * <ol>
   *   <li>Sorts suites by name for consistent processing order
   *   <li>Collects all test results from all suites
   *   <li>Removes duplicates within each result category
   *   <li>Applies hierarchy rules to remove lower-priority duplicates
   * </ol>
   *
   * <h3>Example Usage:</h3>
   *
   ...
   * }</pre>
   *
   * @param suites the list of TestNG suites to process for duplicate removal. Must not be null. The
  ...
  public static synchronized void removeDuplicateResults(CList<ISuite> suites) {
    ...
    removeDuplicate(
        totalPassed, (context, result) -> context.getPassedTests().getAllResults().remove(result));
    removeDuplicate(
        totalPassed,
        (context, result) ->
    ...
    removeDuplicate(
        totalFailed, (context, result) -> context.getFailedTests().getAllResults().remove(result));
    removeDuplicate(
        totalFailed,
        (context, result) ->
            context.getFailedButWithinSuccessPercentageTests().getAllResults().remove(result));
