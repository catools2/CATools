...
/**
 * Log4j2 pattern converter that provides the count of waiting (not yet started) tests
 * in TestNG execution statistics. This converter integrates with the CExecutionStatisticListener
 * to retrieve real-time counts of tests that are queued for execution but haven't started yet.
 * 
 * <p>Waiting tests are calculated as: total tests - passed - failed - skipped - running tests.
 * This gives you visibility into how many tests are still pending execution in your test suite.
 * 
 * <p>The converter is registered as a Log4j2 plugin and can be used in log pattern layouts
 * with the converter key "ETW" (Execution Tests Waiting).
 * 
 ...
 * <ul>
 *   <li>Monitoring test execution progress in real-time logs</li>
 *   <li>Tracking test queue depth during parallel execution</li>
 *   <li>Debugging test execution bottlenecks</li>
 *   <li>Providing visibility into remaining test workload</li>
 * </ul>
 * 
...
public class CTotalWaitingTestCountConverter extends CBaseExecutionStatisticConverter {
   ...
   * CExecutionStatisticListener. The converter will display the count of tests that are
   * queued for execution but haven't started yet.
   * 
   * <p>The string format parameter allows customization of how the waiting test count
   * is displayed in log output. If null or blank, it defaults to "%d".
   *
   * @param stringFormat the format string for displaying the waiting test count.
   *                     Common formats include:
   *                     <ul>
   *                       <li>"%d" - simple integer display (default)</li>
   *                       <li>"%03d" - zero-padded 3-digit display</li>
   *                       <li>"Waiting: %d" - descriptive format</li>
   *                       <li>"%,d" - comma-separated for large numbers</li>
   *                     </ul>
   * 
   * <h3>Examples:</h3>
   * <pre>{@code
   ...
   * 
   * <p>This method is called by the Log4j2 framework when the converter is used in
   * pattern layouts. It validates the provided options and creates a new converter
   * instance with the appropriate string format.
   * 
   * <p>The options array should contain at most one element, which specifies the
   * string format for displaying the waiting test count. If no options are provided
   * or the array is empty, the default format "%d" will be used.
   * 
   * @param options configuration options from the Log4j2 pattern layout.
   *                Expected to contain at most one string format option.
   *                Examples:
   *                <ul>
   *                  <li>null or empty array - uses default "%d" format</li>
   *                  <li>new String[]{"%03d"} - uses zero-padded format</li>
   *                  <li>new String[]{"Waiting: %d"} - uses descriptive format</li>
   *                </ul>
   * 
