...
/**
 * Log4j pattern converter that displays the total count of tests tracked by the execution
 * statistics listener. This converter can be used in Log4j pattern layouts to show real-time test
 * execution statistics.
 *
 * <p>The converter is registered with the key "ETT" and retrieves the total test count from {@link
 * CExecutionStatisticListener#getTotal()}, which includes all tests regardless of their execution
 * status (passed, failed, skipped, running, or waiting).
 *
 * <p>The total count represents the complete number of test methods discovered and tracked during
 * test suite execution, providing visibility into the overall test scope.
 *
...
public class CTotalTestCountConverter extends CBaseExecutionStatisticConverter {
   ...
   *
   * <p>This constructor initializes the converter to display the total test count using the
   * provided format string. The converter will retrieve real-time test count data from {@link
   * CExecutionStatisticListener#getTotal()}.
   *
   * @param stringFormat the format string for displaying the total test count. If null or blank,
   *     defaults to "%d". Common formats include:
   *     <ul>
   *       <li>"%d" - simple integer display (e.g., "127")
   *       <li>"%03d" - zero-padded 3-digit display (e.g., "127")
   *       <li>"Total: %d" - descriptive format (e.g., "Total: 127")
   *     </ul>
   *     <h3>Examples:</h3>
   *     <pre>{@code
   * // Default formatting
   ...
   *
   * <p>This factory method is used by the Log4j framework to instantiate the converter when it
   * encounters the "ETT" pattern key in a layout configuration. The method validates the provided
   * options and creates a converter instance with the appropriate string format.
   *
   * @param options configuration options from the Log4j pattern layout. Can be null, empty, or
   *     contain at most one string format option. If provided, the first element should be a valid
   *     format string (e.g., "%d", "%03d", "Total: %d")
