...
/**
 * A Log4j2 pattern converter that provides the total count of skipped tests during TestNG execution.
 * <p>
 * This converter is used to display the number of tests that have been skipped in log messages.
 * It integrates with the {@link CExecutionStatisticListener} to retrieve real-time statistics
 * about test execution status.
 * </p>
 * 
...
public class CTotalSkippedTestCountConverter extends CBaseExecutionStatisticConverter {
   ...
   * <p>
   * This constructor initializes the converter with a custom string format for displaying
   * the skipped test count. The converter will retrieve the skipped test count from the
   * {@link CExecutionStatisticListener}.
   * </p>
   * 
   * <h4>String Format Examples:</h4>
   * <ul>
   *   <li>{@code "%d"} - Default format, displays as integer (e.g., "5")</li>
   *   <li>{@code "%03d"} - Zero-padded 3-digit format (e.g., "005")</li>
   *   <li>{@code "%,d"} - Comma-separated format (e.g., "1,234")</li>
   *   <li>{@code "Skipped: %d"} - Custom prefix format (e.g., "Skipped: 5")</li>
   * </ul>
   *
   * @param stringFormat format to be used for displaying the skipped count. 
   ...
   * <p>
   * This static factory method is used by Log4j2 to instantiate the converter when
   * it encounters the %ETS pattern in a layout configuration. The method validates
   * the provided options and creates a converter with the appropriate string format.
   * </p>
   * 
   ...
   * <ul>
   *   <li>If options is null or empty: uses default format "%d"</li>
   *   <li>If options contains one element: uses that element as format string</li>
   *   <li>If options contains more than one element: assertion failure (development only)</li>
   * </ul>
   *
   * @param options array of configuration options where the first element (if present)
   *                is used as the string format for displaying the skipped count.
   *                May be null or empty for default formatting.
