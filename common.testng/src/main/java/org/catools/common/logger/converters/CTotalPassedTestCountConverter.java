...
/**
 * A Log4j pattern converter that formats the total count of passed tests in log messages.
 * This converter integrates with the TestNG execution statistics to provide real-time
 * information about test execution progress.
 * 
 * <p>The converter is registered with the key "ETP" (Execution Total Passed) and can be
 * used in Log4j configuration patterns to display the current count of passed tests.</p>
 * 
 ...
 * </pre>
 * 
 * <p><strong>Note:</strong> This converter requires the {@link CExecutionStatisticListener}
 * to be registered as a TestNG listener to track test execution statistics.</p>
 * 
...
public class CTotalPassedTestCountConverter extends CBaseExecutionStatisticConverter {
   ...
   * 
   * <p>This constructor initializes the converter with a custom string format for
   * displaying the passed test count. The format follows standard Java printf-style
   * formatting conventions.</p>
   * 
   * <h4>Format Examples:</h4>
   * <ul>
   *   <li>{@code "%d"} - Simple integer format (default): "5"</li>
   *   <li>{@code "%03d"} - Zero-padded 3-digit format: "005"</li>
   *   <li>{@code "Passed: %d"} - With prefix text: "Passed: 5"</li>
   *   <li>{@code "%d/%d"} - Though only one value available: "5" (second %d ignored)</li>
   * </ul>
   *
   * @param stringFormat format to be used for displaying the count. 
   *                    Defaults to "%d" if null or blank. Must be a valid printf-style format string.
   * 
   ...
   * 
   * <p>This factory method is called by the Log4j framework when parsing configuration
   * files. It validates the provided options and creates a converter instance with
   * the appropriate string format.</p>
   * 
   ...
   *               representing the string format. If null or empty, default format "%d" is used.
