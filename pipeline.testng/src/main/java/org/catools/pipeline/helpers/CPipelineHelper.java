...
import static org.catools.pipeline.cache.CPipelineCacheManager.getExecutor;
import static org.catools.pipeline.cache.CPipelineCacheManager.readExecutionMetaData;
import static org.catools.pipeline.cache.CPipelineCacheManager.readMetaData;
...
/**
 * Helper class for managing pipeline operations and test execution tracking. This class provides
 * utility methods for creating, retrieving, and managing pipelines, as well as adding test
 * execution results to pipelines.
 *
 * <p>The CPipelineHelper class is responsible for:
 *
 * <ul>
 *   <li>Creating and retrieving pipeline instances based on configuration
 *   <li>Adding test execution results to pipelines with associated metadata
 *   <li>Managing pipeline execution metadata and annotations
 * </ul>
 *
 * <p>Example usage:
 *
...
public class CPipelineHelper {
   ...
   *
   * <ul>
   *   <li>If pipeline functionality is disabled, returns null
   *   <li>If configured to always create new pipelines, creates a new pipeline
   *   <li>Otherwise, attempts to retrieve the last pipeline by name
   *   <li>If no existing pipeline is found and creation is allowed, creates a new one
   * </ul>
   *
   * @return the retrieved or newly created pipeline, or null if pipeline functionality is disabled
   * @throws NullPointerException if no pipeline with the configured name is found and automatic
  ...
  /**
   * Adds a test execution result to the specified pipeline with empty metadata list. This is a
   * convenience method that calls {@link #addExecution(CPipeline, CTestResult, List)} with an empty
   * metadata list.
   *
  ...
  /**
   * Adds a test execution result to the specified pipeline along with additional execution
   * metadata.
   *
   * <p>This method processes the test result and extracts various types of metadata including:
   *
   * <ul>
   *   <li>Test IDs, defect IDs, and deferred IDs
   *   <li>Method groups and test parameters
   *   <li>Awaiting status and regression depth
   *   <li>Severity level and host information
   *   <li>Custom annotations and execution metadata
  ...
  public static void addExecution(
      ...
      List<CPipelineExecutionMetaData> executionMetaData) {
    ...
    if (testResult.getTestIds() != null)
      testResult
          .getTestIds()
          .forEach(t -> executionMetaData.add(readExecutionMetaData("Test ID", t)));

    if (testResult.getOpenDefectIds() != null)
      testResult
          .getOpenDefectIds()
          .forEach(t -> executionMetaData.add(readExecutionMetaData("Open Defect ID", t)));

    if (testResult.getDefectIds() != null)
      testResult
          .getDefectIds()
          .forEach(t -> executionMetaData.add(readExecutionMetaData("Defect ID", t)));

    if (testResult.getDeferredId() != null)
      testResult
          .getDeferredId()
          .forEach(t -> executionMetaData.add(readExecutionMetaData("Deferred ID", t)));

    if (testResult.getMethodGroups() != null)
      testResult
          .getMethodGroups()
          .forEach(t -> executionMetaData.add(readExecutionMetaData("Method Group", t)));

    if (testResult.getParameters() != null && testResult.getParameters().length > 0)
      executionMetaData.add(
          readExecutionMetaData("Parameters", CJsonUtil.toString(testResult.getParameters())));
    ...
    if (testResult.getRegressionDepth() != null)
      executionMetaData.add(
          readExecutionMetaData("Regression Depth", testResult.getRegressionDepth().toString()));

    if (testResult.getSeverityLevel() != null)
      executionMetaData.add(
          readExecutionMetaData("Severity Level", testResult.getSeverityLevel().toString()));
    ...
    if (testResult.getExceptionInfo() != null)
      execution.setException(
          new CPipelineExecutionException(
    ...
    if (testResult.getExecutionMetadata() != null)
      testResult
          ...
          .forEach(m -> executionMetaData.add(readExecutionMetaData(m.getName(), m.getValue())));

    execution
        .setPipeline(pipeline)
        .setStatus(
            new CPipelineStatus(testResult.getStatus().getId(), testResult.getStatus().name()))
  ...
  /**
   * Determines whether certain annotations should be skipped during metadata extraction. This
   * method filters out annotations that are already processed elsewhere to avoid duplication.
   *
   * <p>The following annotations are skipped:
   *
   * <ul>
   *   <li>{@code @Test} - TestNG's basic test annotation
   *   <li>{@code @CRegression} - Custom regression annotation
   *   <li>{@code @CSeverity} - Custom severity annotation
   *   <li>{@code @CDefects} - Custom defects annotation
   *   <li>{@code @CAwaiting} - Custom awaiting annotation
   *   <li>{@code @COpenDefects} - Custom open defects annotation
   *   <li>{@code @CDeferred} - Custom deferred annotation
   *   <li>{@code @CTestIds} - Custom test IDs annotation
   *   <li>{@code @CIgnored} - Custom ignored annotation
   * </ul>
   *
  ...
  /**
   * Creates and builds a new pipeline instance based on current configuration settings. This method
   * handles the complete pipeline creation process including project setup and metadata
   * association.
   *
   * <p>The pipeline creation process includes:
   *
   * <ul>
   *   <li>Merging the project configuration into the database
   *   <li>Processing and converting pipeline metadata
   *   <li>Creating a new pipeline with all configuration properties
   *   <li>Persisting the pipeline to the database
   * </ul>
   *
  ...
  private static CPipeline buildPipeline() {
    ...
    List<CPipelineMetaData> metaData =
        CPipelineConfigs.getPipelineMetaData().stream()
