...
/**
 * Utility class for managing pipeline TestNG configuration settings. This class provides access to
 * configuration properties that control the behavior of pipeline listeners in TestNG test execution
 * environments.
 *
 * <p>Configuration properties are loaded from HOCON configuration files and can be overridden using
 * system properties or environment variables.
 *
 * <h3>Configuration Properties:</h3>
 *
 * <ul>
 *   <li>{@code catools.pipeline.listener.enabled} - Enables/disables pipeline listener
 *   <li>{@code catools.pipeline.listener.always_create_new_pipeline} - Forces creation of new
 *       pipeline
 *   <li>{@code catools.pipeline.listener.create_if_not_exist} - Creates pipeline if it doesn't
 *       exist
 * </ul>
 *
...
public class CPipelineTestNGConfigs {
   ...
   *
   * <p>When enabled, the pipeline listener will be active during TestNG test execution and will
   * capture test execution data for pipeline tracking and reporting.
   *
   ...
   * you want to maintain separate pipeline instances for different test runs or environments.
   *
   * <h4>Configuration Property:</h4>
   *
   ...
   *
   * <p>When this option is enabled, the system will check for the existence of a pipeline and
   * create one only if it doesn't exist. This is the recommended approach for most scenarios as it
   * ensures pipeline continuity while avoiding unnecessary duplicates.
   *
   * <p><strong>Note:</strong> This setting is ignored if {@link #always_create_new_pipeline()}
   * returns {@code true}, as the latter takes precedence.
   *
   * <h4>Configuration Property:</h4>
   *
   ...
   * }</pre>
   *
   * @return {@code true} if a pipeline should be created when it doesn't exist, {@code false}
   *     otherwise
   */
  ...
  private enum Configs implements CHoconPath {
    ...
    CATOOLS_PIPELINE_LISTENER_ALWAYS_CREATE_NEW_PIPELINE(
        "catools.pipeline.listener.always_create_new_pipeline"),
