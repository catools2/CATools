 ...
 * in a collection. This interface extends {@link CIterable} with dynamic extension capabilities for
 * web-based testing scenarios.
 *
...
public interface CWebIterable<E> extends CIterable<E, Iterable<E>>, CDynamicIterableExtension<E> {
   ...
   *
   * <p>The index is zero-based, meaning the first element has index 0, the second element has index
   * 1, and so on. This method provides direct access to elements without needing to iterate through
   * the collection.
   *
   * <p><strong>Example usage:</strong>
   *
   ...
   *
   * <p>This method is useful for validating whether an element exists before attempting to retrieve
   * it with {@link #getRecord(int)}. It uses zero-based indexing where the first element has index
   * 0.
   *
   * <p>This method should be used for bounds checking and conditional logic to avoid exceptions
   * when accessing elements that may not exist.
   *
   * <p><strong>Example usage:</strong>
   *
   * <pre>{@code
   ...
   *
   * <p>This method provides access to the raw iterable collection that backs this {@code
   * CWebIterable}. It's primarily used internally by the framework but can be useful when you need
   * to pass the collection to methods expecting a standard {@link Iterable}.
   *
   * <p><strong>Example usage:</strong>
   *
   * <pre>{@code
   ...
   *
   * <p>This method returns {@code true} to enable waiting functionality, which is essential for web
   * automation where elements may take time to load or become available. The framework uses this to
   * determine whether to apply wait strategies when accessing elements.
   *
   * <p>For web elements, waiting is typically necessary because:
   *
   * <ul>
   *   <li>Elements may load asynchronously after page load
   *   <li>Dynamic content may be added via JavaScript
   *   <li>Elements may need time to become visible or interactable
   * </ul>
   *
   ...
   *
   * <p>This method provides a standard {@link Iterator} implementation that allows traversing
   * through all web elements in the collection. The iterator uses the {@link #hasRecord(int)} and
   * {@link #getRecord(int)} methods internally to provide sequential access to elements.
   *
   * <p>The iterator supports the standard iteration pattern and will throw {@link
   * NoSuchElementException} if {@link Iterator#next()} is called when no more elements are
   * available. The {@link Iterator#remove()} operation is not supported and will throw {@link
   ...
   * @throws NoSuchElementException when {@link Iterator#next()} is called but no more elements
   *     exist
