 ...
 */
@Slf4j
public class CWebSelect<DR extends CDriver> extends CWebElement {

  /**
   * Constructs a CWebSelect with the specified name, driver, and locator. Uses the default wait
   * time configured in the parent class.
   *
  ...
  /**
   * Dynamic extension for accessing the currently selected option's text. Provides verification
   * capabilities and automatic retrieval of the selected text.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Dynamic extension for accessing the currently selected option's value attribute. Provides
   * verification capabilities and automatic retrieval of the selected value.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Dynamic extension for accessing all available option values in the select element. Provides
   * verification capabilities and automatic retrieval of all option values.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Dynamic extension for accessing all available option texts in the select element. Provides
   * verification capabilities and automatic retrieval of all option texts.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Dynamic extension for accessing all currently selected option texts (for multi-select
   * elements). Provides verification capabilities and automatic retrieval of selected option texts.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Dynamic extension for accessing all currently selected option values (for multi-select
   * elements). Provides verification capabilities and automatic retrieval of selected option
   * values.
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Gets all currently selected option texts using the default wait time. For single-select
   * elements, returns a list with one item. For multi-select elements, returns all selected
   * options.
   *
  ...
  public String getSelectedText(int waitSecs) {
    return waitUntil(
        "Get Selected Text",
        waitSecs,
        engine -> engine.getFirstSelectedOption(getLocator()).text().trim());
  }
  ...
  public String getSelectedValue(int waitSecs) {
    return waitUntil(
        "Get Selected Value",
        waitSecs,
        engine -> engine.getFirstSelectedOption(getLocator()).getAttribute("value").trim());
  }
  ...
  public CList<String> getTexts(int waitSecs) {
    return waitUntil(
        "Get Text",
        waitSecs,
        engine -> new CList<>(engine.getOptions(getLocator())).mapToList(e -> e.text().trim()));
  }
  ...
  /**
   * Selects the first option in the select element using the default wait time. This is equivalent
   * to selectByIndex(0).
   *
   * @example
   *     <pre>{@code
  ...
  /**
   * Selects an option by partial text match using the default wait time. Searches for options
   * containing the specified pattern anywhere in their text.
   *
  ...
  /**
   * Selects an option by its visible text using the default wait time. The text must match exactly
   * (case-sensitive).
   *
  ...
  /**
   * Selects an option by its index position using the default wait time. Index starts from 0 for
   * the first option.
   *
  ...
  /**
   * Selects an option using a regular expression pattern to match option text using the default
   * wait time. The pattern is applied against the trimmed text of each option.
   *
  ...
  /**
   * Selects an option using a regular expression pattern to match option text with a custom wait
   * time.
   *
   * @param pattern the regular expression pattern to match against option texts
   * @param waitSec maximum time to wait for the element in seconds
   ...
   * }</pre>
   */
  public boolean selectByTextPattern(String pattern, int waitSec) {
    return waitUntil(
        "Select By Text Pattern",
        waitSec,
        engine -> engine.selectByTextPattern(getLocator(), pattern));
  }

  /**
   * Sets the selection state based on a boolean value using the default wait time. If true, selects
   * the element; if false, deselects it. This method is useful for conditional selection logic.
   *
