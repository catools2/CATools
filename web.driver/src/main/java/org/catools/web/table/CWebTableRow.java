 ...
 * provides functionality to interact with table cells within a specific row, supporting both simple
 * text retrieval and complex control interactions.
 *
 * <p>Example usage:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * @param <EE> the driver type extending CDriver
...
public abstract class CWebTableRow<EE extends CElementEngine<?>, P extends CWebTable<EE, ?>>
    extends CWebElement implements CWebComponent {
  ...
  @Setter private String cellXpath;
   ...
   * }</pre>
   */
  public CWebTableRow(String name, EE driver, int idx, P parentTable) {
   ...
   * }</pre>
   */
  public CWebTableRow(String name, EE driver, int rowIndex, P parentTable, int waitSec) {
    super(name, driver, CBy.xpath(parentTable.getRowXpath(rowIndex)), waitSec);
  ...
  /**
   * Reads all cells in this row and returns their content and metadata. This method iterates
   * through all columns defined in the parent table's header map and extracts the text content and
   * visibility status of each cell.
   *
   * @return a list of CWebTableCell objects containing cell data including index, header name, text
  ...
  public CList<CWebTableCell> readRowCells() {
    ...
    for (Integer idx : headersMap.keySet()) {
      ...
      CWebElement element = getCell(header);
  ...
  /**
   * Creates a CWebElement that points to a specific cell within this row. This method allows access
   * to cells with duplicate header names by using an index, and supports child locators for
   * accessing nested DOM elements within the cell.
   *
   ...
   * }</pre>
   */
  protected CWebElement getCell(String header, int index, String childLocator) {
    return new CWebElement(header, elementEngine, getCellLocator(header, index, childLocator));
  }

  /**
   * Creates a CWebElement that points to the first cell with the specified header name. This is a
   * convenience method equivalent to calling getCell(header, 0).
   *
   * @param header the column header name to locate the cell
   ...
   * }</pre>
   */
  protected CWebElement getCell(String header) {
    return getCell(header, 0);
  }

  /**
   * Creates a CWebElement that points to a specific cell with the given header and index. This
   * method is useful when there are multiple columns with the same header name.
   *
   * @param header the column header name to locate the cell
   * @param index the zero-based index of the header (used when multiple columns have the same name)
   * @return a CWebElement instance pointing to the specified cell
   ...
   * }</pre>
   */
  protected CWebElement getCell(String header, int index) {
    return getCell(header, index, CStringUtil.EMPTY);
  }

  /**
   * Creates a custom control instance using a control builder function for the first cell with the
   * specified header. This method enables creation of specialized web controls (like buttons,
   * dropdowns, checkboxes) from table cells.
   *
   * @param <C> the type of control to be created
   * @param header the column header name to locate the cell
   * @param controlBuilder function that creates a control instance from a CBy locator
   ...
   * }</pre>
   */
  protected <C> C getCell(String header, Function<CBy, C> controlBuilder) {
   ...
   * identified by header and index. This method is useful when there are multiple columns with the
   * same header name and you need to create specialized controls.
   *
   * @param <C> the type of control to be created
   * @param header the column header name to locate the cell
   * @param index the zero-based index of the header (used when multiple columns have the same name)
   ...
   * }</pre>
   */
  protected <C> C getCell(String header, int index, Function<CBy, C> controlBuilder) {
    return getCell(header, index, CStringUtil.EMPTY, controlBuilder);
  }

  /**
   * Creates a custom control instance using a control builder function for the first cell with the
   * specified header, including a child locator for accessing nested elements. This method combines
   * header-based cell location with child element access.
   *
   * @param <C> the type of control to be created
   * @param header the column header name to locate the cell
   * @param childLocator additional XPath locator for accessing child elements within the cell
   ...
   * }</pre>
   */
  protected <C> C getCell(String header, String childLocator, Function<CBy, C> controlBuilder) {
    return getCell(header, 0, childLocator, controlBuilder);
  }

  /**
   * Creates a custom control instance using a control builder function for a specific cell. This is
   * the most comprehensive method that allows specification of header, index, child locator, and
   * control builder, providing maximum flexibility for complex table interactions.
   *
   * @param <C> the type of control to be created
   * @param header the column header name to locate the cell
   * @param index the zero-based index of the header (used when multiple columns have the same name)
   ...
   * }</pre>
   */
  protected <C> C getCell(
      String header, int index, String childLocator, Function<CBy, C> controlBuilder) {
    return controlBuilder.apply(getCellLocator(header, index, childLocator));
  }

  /**
   * Constructs a CBy locator for a specific cell within this table row. This method handles the
   * complex logic of mapping header names to column indices, supporting duplicate header names and
   * child element access within cells.
   *
   * <p>The method performs the following operations:
   *
   * <ul>
   *   <li>Searches for all columns matching the specified header name
   *   <li>Validates that the requested index exists
   *   <li>Constructs an XPath locator combining row and cell positioning
   *   <li>Appends child locator if provided
   * </ul>
   *
   ...
   * @return a CBy locator that can be used to find the specific cell element
   * @throws IllegalArgumentException if the header is not found in the table or if the index is
   *     greater than the number of matching headers
   * @example
   *     <pre>{@code
   * // Get locator for a simple cell
   * CBy cellLocator = row.getCellLocator("Product Name", 0, "");
   *
   * // Get locator for a nested button within a cell
   * CBy buttonLocator = row.getCellLocator("Actions", 0, "//button[@class='edit']");
   *
   * // Get locator for second column with same header
   * CBy secondStatusLocator = row.getCellLocator("Status", 1, "");
   * }</pre>
   */
  protected CBy getCellLocator(String header, int index, String childLocator) {
    CHashMap<Integer, String> allMatches =
        parentTable.getHeadersMap().getAll((k, v) -> StringUtils.equals(header, v));

    if (allMatches.isEmpty() || allMatches.size() < index) {
      // We send invalid locator in case if header does not exist
      throw new IllegalArgumentException(
          "Header not found, header:'" + header + "', index:" + index);
    }
    ...
    return CBy.xpath(getLocator() + "/./" + cellLocator);
  }
}
