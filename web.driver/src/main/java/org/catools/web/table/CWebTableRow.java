...
public abstract class CWebTableRow<DR extends CDriver, P extends CWebTable<DR, ?>>
   ...
   * }</pre>
   */
  protected <C> C getCell(String header, int index, String childLocator, Function<CBy, C> controlBuilder) {
    return controlBuilder.apply(getCellLocator(header, index, childLocator));
  }

  /**
   * Constructs a CBy locator for a specific cell within this table row.
