...
/**
 * A comprehensive web performance metrics collector that implements {@link CDriverListener} to
 * automatically capture and aggregate performance data during web driver interactions.
 *
 * <p>This listener provides thread-safe collection of web performance metrics by automatically
 * tracking driver actions, page transitions, and page load operations. It maintains separate metric
 * collections per thread using {@link ThreadLocal} storage, making it suitable for concurrent test
 * execution scenarios.
 *
 * <p>The listener automatically collects three main categories of performance data:
 *
 * <ul>
 *   <li><strong>Action Metrics</strong> - Performance timing for individual web actions (clicks,
 *       form fills, etc.)
 *   <li><strong>Page Load Metrics</strong> - Timing data for page loading operations
 *   <li><strong>Page Transition Metrics</strong> - Information about page navigations and state
 *       changes
 * </ul>
 *
 * <h3>Key Features:</h3>
 *
 * <ul>
 *   <li>Thread-safe operation using ThreadLocal storage
 *   <li>Automatic memory cleanup via shutdown hook
 *   <li>Seamless integration with existing WebDriver test frameworks
 *   <li>Zero-configuration setup - just register the listener
 *   <li>Comprehensive performance data collection
 * </ul>
 *
...
public class CWebMetricCollectorListener implements CDriverListener {
  private final ThreadLocal<CWebMetric> pageMetricThreadLocal =
      ThreadLocal.withInitial(CWebMetric::new);

  /**
   * Constructs a new CWebMetricCollectorListener with automatic memory management.
   *
   * <p>This constructor initializes the thread-local storage for metrics collection and registers a
   * shutdown hook to ensure proper cleanup of thread-local resources when the JVM terminates. This
   * prevents potential memory leaks in long-running applications or test suites.
   *
   * <p>The shutdown hook automatically calls {@code ThreadLocal.remove()} to clean up any
   * thread-local data that may still be present when the application shuts down, ensuring proper
   * resource management even if metrics are not explicitly cleared.
   *
   * <h4>Memory Management Features:</h4>
   *
   * <ul>
   *   <li>Automatic ThreadLocal cleanup via shutdown hook
   *   <li>Prevention of memory leaks in long-running applications
   *   <li>Safe for use in application servers and test frameworks
   *   <li>No manual cleanup required in most scenarios
   * </ul>
   *
   ...
   *
   * <p>This method provides access to the thread-local {@link CWebMetric} instance that has been
   * automatically populated by the listener during web driver interactions. Each thread maintains
   * its own separate metrics collection, ensuring thread safety in concurrent testing scenarios.
   *
   * <p>The returned {@link CWebMetric} object contains three categories of performance data:
   *
   * <ul>
   *   <li><strong>Action Performance</strong> - Timing and details for individual web actions
   *   <li><strong>Page Load Metrics</strong> - Performance data for page loading operations
   *   <li><strong>Page Transition Information</strong> - Details about page navigation events
   * </ul>
   *
   * <p>The metrics are automatically collected as the listener responds to WebDriver events,
   * requiring no manual intervention from the test code. This enables transparent performance
   * monitoring without modifying existing test logic.
   *
   * <h4>Thread Safety:</h4>
   *
   * <p>This method is completely thread-safe. Each calling thread receives its own isolated metrics
   * collection, preventing data contamination in multi-threaded test execution environments.
   *
   * <h4>Usage Examples:</h4>
   *
   ...
   * }</pre>
   *
   * @return A {@link CWebMetric} instance containing all performance metrics collected for the
   *     current thread. This includes action timings, page load data, and transition information.
   *     Never returns null - will return an empty metrics object if no data has been collected yet.
   * @see CWebMetric
   * @see ThreadLocal
   ...
   *
   * <p>This callback method is invoked by the driver framework after every web interaction (such as
   * clicks, form fills, navigation, etc.) to automatically collect action-specific performance
   * data. It creates and stores a comprehensive action metric that includes timing information,
   * page state changes, and action metadata.
   *
   * <p>The method leverages the provided parameters to create a detailed performance record without
   * requiring any manual intervention from test code, enabling transparent and comprehensive
   * performance monitoring across all web driver interactions.
   *
   * <h4>Automatic Data Collection:</h4>
   *
   * <p>This method automatically records:
   *
   * <ul>
   *   <li>Action execution timing (calculated from start time to current time)
   *   <li>Page state before the action (URL, title, DOM state)
   *   <li>Page state after the action (changes in URL, title, DOM state)
   *   <li>Action name and metadata for later analysis
   *   <li>Thread-specific metric storage for concurrent testing
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>WebDriver wrapper implementations
   *   <li>Test framework integration layers
   *   <li>Driver proxy or interceptor mechanisms
   *   <li>Event-driven automation frameworks
   * </ul>
   *
   * @param actionName A descriptive name identifying the specific action that was performed.
   *     Examples: "Click Login Button", "Fill Username Field", "Submit Form", "Navigate to
   *     Dashboard". This name appears in performance reports and analysis output for easy
   *     identification.
   * @param engine The CDriverEngine instance that executed the action. Used for context and
   *     potential additional data extraction if needed.
   * @param pageBeforeAction Complete page state information captured before the action was
   *     executed, including URL, title, DOM elements, and other relevant page metadata. Used to
   *     establish the starting context.
   * @param pageAfterAction Complete page state information captured after the action was completed,
   *     including any changes to URL, title, DOM elements, or page content. Used to measure the
   *     impact of the action.
   * @param driverMetricInfo Detailed transition information containing performance metrics and
   *     metadata about the driver operation. May include network timing, rendering data, and other
   *     driver-level performance indicators.
   * @param startTime The precise timestamp when the action was initiated. Used to calculate the
   *     total action duration with nanosecond precision for accurate performance measurement.
   * @param durationInNano The total duration of the action execution in nanoseconds. This parameter
   *     provides the exact timing measurement for the operation, enabling precise performance
   *     analysis.
   ...
   * }</pre>
   *
   * @see CDriverListener#afterAction(String, CDriverEngine, CWebPageInfo, CWebPageInfo,
   *     CWebPageTransitionInfo, CDate, long)
   ...
   *
   * <p>This callback method is invoked by the driver framework whenever a page transition is
   * detected, such as navigation events, form submissions that change pages, or dynamic content
   * updates that alter the page state. It automatically records both page loading performance and
   * transition metadata for comprehensive page-level performance analysis.
   *
   * <p>The method performs two distinct types of metric collection:
   *
   * <ol>
   *   <li><strong>Page Load Metrics</strong> - Timing data for the page loading operation
   *   <li><strong>Page Performance Data</strong> - Detailed transition information and metadata
   * </ol>
   *
   * <p>This dual approach provides both quantitative performance measurements (timing) and
   * qualitative information (what changed, how the page transitioned) for thorough performance
   * analysis and debugging.
   *
   * <h4>Automatic Metric Types Collected:</h4>
   *
   * <ul>
   *   <li><strong>Page Load Timing</strong> - Complete loading duration with nanosecond precision
   *   <li><strong>Page Metadata</strong> - Title, URL, and page state information
   *   <li><strong>Transition Context</strong> - Details about what triggered the page change
   *   <li><strong>Performance Indicators</strong> - Network, rendering, and processing metrics
   * </ul>
   *
   * <h4>Common Page Change Triggers:</h4>
   *
   * <ul>
   *   <li>Direct navigation via driver.get() or driver.navigate()
   *   <li>Form submissions that result in page redirects
   *   <li>Link clicks that navigate to new pages
   *   <li>JavaScript-triggered page changes or SPA route changes
   *   <li>Browser back/forward navigation
   *   <li>Page refreshes or reloads
   * </ul>
   *
   * @param engine The CDriverEngine instance where the page change occurred. Used to extract
   *     current page information (title, URL) for the page load metric recording.
   * @param pageTransitionInfo Comprehensive information about the page transition, including
   *     before/after states, transition type, performance metrics, and metadata about what caused
   *     the page change. This object contains detailed context for analysis.
   * @param startTime The precise timestamp when the page transition was initiated. Used to
   *     calculate the total page loading duration with nanosecond precision for accurate
   *     performance measurement.
   * @param durationInNano The total duration of the page transition in nanoseconds. Represents the
   *     complete time from initiation to completion of the page change operation.
   *     <h4>Usage Context Examples:</h4>
  ...
  public void onPageChanged(
      CDriverEngine engine,
      CWebPageTransitionInfo pageTransitionInfo,
      CDate startTime,
      long durationInNano) {
