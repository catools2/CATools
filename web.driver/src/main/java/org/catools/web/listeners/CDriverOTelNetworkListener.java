...
/**
 * OpenTelemetry-enabled network listener for Selenium WebDriver that tracks HTTP requests and responses
 * using Chrome DevTools Protocol. This listener provides comprehensive network monitoring and tracing
 * capabilities for web automation tests.
 * 
 * <p>The listener captures detailed information about network requests including:</p>
 * <ul>
 *   <li>Request method, URL, headers, and timing</li>
 *   <li>Response status codes, MIME types, and headers</li>
 ...
 * capabilities and integration with observability platforms.</p>
 *
 ...
 * <ul>
 *   <li>Network metrics collection must be enabled via {@link CDriverConfigs#isCollectNetworkMetricsEnable()}</li>
 *   <li>OpenTelemetry must be enabled via {@link COTelConfig#isEnable()}</li>
 *   <li>Chrome or Edge WebDriver is required (supports Chrome DevTools Protocol)</li>
 * </ul>
 *
 * <p><strong>Thread Safety:</strong></p>
 * <p>This class is thread-safe and uses concurrent collections to handle multiple simultaneous
 * network requests. Each request is tracked independently using request IDs.</p>
 * 
 * @see DevTools
 * @see COTelConfig
 * @see CDriverConfigs
...
public class CDriverOTelNetworkListener {
   ...
   * 
   * <p>This constructor initializes the tracer and creates a parent span that will contain
   * all network request spans. The parent span represents the overall action or test scenario
   * being traced.</p>
   * 
   * @param tracerName the name of the OpenTelemetry tracer to use for creating spans.
   *                   This should be descriptive of the component or module being traced
   *                   (e.g., "WebDriverTracer", "SeleniumTests")
   * @param actionName the name of the parent span that represents the high-level action
   *                   being performed (e.g., "LoginTest", "CheckoutProcess", "PageLoad").
   *                   All network requests will be traced as children of this span
   * 
   ...
   * 
   * <p>This method registers event listeners for three types of network events:</p>
   * <ul>
   *   <li><strong>Request Will Be Sent:</strong> Captures outgoing requests with method, URL, and headers</li>
   *   <li><strong>Response Received:</strong> Captures response data including status, timing, and headers</li>
   *   <li><strong>Loading Failed:</strong> Handles network errors and connection failures</li>
   * </ul>
   * 
   * <p>Each network request is traced as an individual span with detailed attributes.
   * The method automatically handles span lifecycle management and parent-child relationships.</p>
   * 
   * @param devTools the Chrome DevTools instance to attach listeners to. Must be from a 
   *                 Chrome or Edge WebDriver with an active DevTools session
   * 
   ...
   * <p>The listener will only attach if both network metrics collection and OpenTelemetry
   * are enabled in the configuration. If either is disabled, this method returns immediately
   * without attaching any listeners.</p>
   * 
   * <h4>Traced Attributes:</h4>
   * <p>The following attributes are automatically captured for each request:</p>
   * <ul>
   *   <li><code>method</code> - HTTP method (GET, POST, etc.)</li>
   *   <li><code>schema</code> - URL scheme (http, https)</li>
   *   <li><code>host</code> - Target hostname</li>
   *   <li><code>path</code> - URL path</li>
   *   <li><code>port</code> - Target port number</li>
   *   <li><code>status</code> - HTTP response status code</li>
   *   <li><code>mime</code> - Response content type</li>
   *   <li><code>protocol</code> - HTTP protocol version</li>
   *   <li><code>latency_ms</code> - Request/response timing in milliseconds</li>
   *   <li><code>header.*</code> - All request and response headers</li>
   * </ul>
   * 
   * @see #detach()
   ...
   * 
   * <p>This method should be called when network monitoring is no longer needed.
   * It properly closes the parent span, ensuring all trace data is correctly
   * recorded in the OpenTelemetry backend.</p>
   * 
   * <p><strong>Important:</strong> Failing to call this method may result in incomplete
   * traces or resource leaks. Always call this method in a finally block or 
   * equivalent cleanup mechanism.</p>
   * 
   * <h4>Usage Example:</h4>
   * <pre>{@code
   * CDriverOTelNetworkListener listener = new CDriverOTelNetworkListener(
   ...
   * <p>Similar to {@link #attach(DevTools)}, this method will only perform cleanup
   * if network metrics collection and OpenTelemetry are both enabled. If either
   * is disabled, this method returns immediately.</p>
   * 
   * @see #attach(DevTools)
   * @see CDriverConfigs#isCollectNetworkMetricsEnable()
   * @see COTelConfig#isEnable()
