...
/**
 * A specialized collection for managing web page load metrics that extends {@link CList} functionality.
 * This class provides a convenient way to store, manipulate, and analyze multiple {@link CWebPageLoadMetric} instances.
 *
 * <p>The class inherits all the enhanced collection operations from {@link CList}, including filtering,
 * searching, partitioning, and conversion operations specifically tailored for web page load metrics.</p>
 *
...
public class CWebPageLoadMetrics extends CList<CWebPageLoadMetric> {
    ...
    public CWebPageLoadMetrics getMetricsInTimeRange(Date startTime, Date endTime) {
        return new CWebPageLoadMetrics(getAll(metric -> {
    ...
    public CWebPageLoadMetrics getMetricsByUrlPattern(String urlPattern) {
        ...
        return new CWebPageLoadMetrics(getAll(metric -> 
            metric.getUrl() != null && metric.getUrl().matches(regex)));
    }
    ...
    public CWebPageLoadMetric getSlowestLoadingPage() {
        return stream()
            .max((m1, m2) -> Long.compare(m1.getDuration(), m2.getDuration()))
            .orElse(null);
    }
    ...
    public CWebPageLoadMetric getFastestLoadingPage() {
        return stream()
            .min((m1, m2) -> Long.compare(m1.getDuration(), m2.getDuration()))
            .orElse(null);
    }
