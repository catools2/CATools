 ...
 * interface offers methods for browser navigation (back/forward), window/frame switching, and page
 * opening operations.
 *
 * <p>All navigation methods support method chaining and provide overloaded versions with retry
 * mechanisms and post-condition validation.
 *
 * @author CATools Team
 * @since 1.0
 */
public interface CDriverNavigation extends CDriverWaiter {
  ...
  /**
   * Navigates the browser back to the previous engine with a post-condition check. Uses default
   * retry settings: 3 attempts with 1 second intervals.
   *
   * @param <T> the type of the implementing class for method chaining
  ...
  /**
   * Navigates the browser back to the previous engine with custom retry settings and post-condition
   * check.
   *
   * @param <T> the type of the implementing class for method chaining
   * @param postCondition predicate to verify the navigation was successful
  ...
  /**
   * Navigates the browser forward to the next engine with a post-condition check. Uses default
   * retry settings: 3 attempts with 1 second intervals.
   *
   * @param <T> the type of the implementing class for method chaining
   * @param postCondition predicate to verify the navigation was successful
  ...
  /**
   * Navigates the browser forward to the next engine with custom retry settings and post-condition
   * check.
   *
   * @param <T> the type of the implementing class for method chaining
   * @param postCondition predicate to verify the navigation was successful
  ...
  /**
   * Switches to a browser window/tab with the specified title using regex matching. If the current
   * window already has the matching title, no switch occurs.
   *
  ...
  /**
   * Switches to the first opened browser window/tab. This is typically the original window that was
   * opened when the session started.
   *
   * @example
   *     <pre>
  ...
  /**
   * Switches to the most recently opened browser window/tab. This is useful for handling new
   * windows opened by JavaScript or link clicks.
   *
   * @example
   *     <pre>
  ...
  /**
   * Switches to a different browser window/tab (not the current one). If only one window is open,
   * the method returns false and no switch occurs.
   *
   * @example
   *     <pre>
  ...
  /**
   * Switches to a frame using the frame's name or id attribute. Uses the default timeout for frame
   * availability.
   *
   * @param locator the name or id of the target frame
   * @example
   *     <pre>
   ...
   * </pre>
   */
  default void switchToFrame(String locator) {
   ...
   * </pre>
   */
  default void switchToFrame(String locator, int waitSec) {
    waitUntil(
        "Switch To Frame",
        waitSec,
        engine -> {
          if (engine != null) engine.switchToFrame(locator);
          return true;
        });
  }

  /**
   * Opens a web engine at the specified URL without returning a engine object. If no browser
   * session is active, a new session will be started automatically.
   *
