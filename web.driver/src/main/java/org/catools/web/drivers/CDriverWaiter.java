...
/**
 * Interface providing advanced waiting capabilities for CDriverEngine operations.
 ...
 *
 * <p>The waiter automatically handles various CDriverEngine exceptions and provides
 ...
 *   <li>Automatic exception handling for common CDriverEngine issues</li>
...
public interface CDriverWaiter {
  ...
  /**
   * Waits until the specified condition is satisfied using the given timeout,
   * returning a default value if the condition times out.
   *
   * Success rules:
   * - If the consumer returns null -> not successful (keep waiting)
   * - If the consumer returns a Boolean -> only Boolean.TRUE is success
   * - Otherwise any non-null return is considered success
   *
   ...
   */
  default <C> C waitUntil(String actionName, int waitSec, C defaultTo, Function<CDriverEngine, C> consumer) {
    try {
      return performActionOnEngine(actionName, engine -> {
        ...
        while (true) {
          try {
            ...
            lastError = t;
            logger.debug("Ignored exception while waiting for {}: {}", actionName, t.getMessage());
          }
   ...
   * returning null if the condition times out.
   *
   * Delegates to the main overload with null as the default value.
   *
   * @param <C>        the return type of the condition
   * @param actionName a descriptive name for the action being performed (for logging)
   * @param waitSec    the maximum time to wait in seconds
  ...
  /**
   * Performs an action on the CDriverEngine instance managed by this session.
   * This method provides safe access to the underlying CDriverEngine with proper
