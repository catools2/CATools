 ...
 *
 * <p>Provides session initialization, state management, and cleanup operations.
 *
 * @author CATools Team
 * @since 1.0
 */
...
public class CDriverSession {
  ...
  @Setter @Getter private CDriverEngine engine;
   ...
   *
   * <p>Must be called before performing any driver actions. It creates the driver engine instance
   * using the configured provider and applies the specified window dimensions and position.
   *
  ...
  /**
   * Resets the session by closing the page (if present) and clearing the reference. Safe to call
   * multiple times.
   */
  public void reset() {
    try {
   ...
   *
   * <ul>
   *   <li>Automatic page information updates
   *   <li>Before/after action event notifications to listeners
   *   <li>DevTools metrics collection (when supported)
   *   <li>Performance timing measurement
   *   <li>Alert handling to prevent interference
   * </ul>
   *
   ...
   * @param actionName a descriptive name for the action being performed (used in logging and
   *     metrics)
