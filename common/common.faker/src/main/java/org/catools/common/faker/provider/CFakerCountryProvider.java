 ...
 *
 * <p>This class serves as a central provider for generating fake data specific to a particular
 * country. It aggregates various specialized providers for names, companies, addresses, and states
 * to provide comprehensive country-localized fake data generation capabilities.
 *
 * <p>The class supports method chaining through the {@code @Accessors(chain = true)} annotation,
 * allowing for fluent-style configuration.
 *
...
public class CFakerCountryProvider {
   ...
   *
   * <p>Contains metadata about the country such as country code and name, which is used to
   * contextualize the fake data generation.
   *
   * @see CRandomCountry
   */
   ...
   *
   * <p>Handles generation of fake data related to states, provinces, or other administrative
   ...
   *
   * <p>Generates culturally appropriate first names, last names, and full names specific to the
   ...
   *
   * <p>Generates fake company names, business types, and other corporate information appropriate
   ...
   *
   * <p>Generates complete addresses including street addresses, using the country-specific
   ...
   *
   * <p>This constructor initializes all the necessary providers for generating country-specific
   * fake data. The address provider is automatically created using the current country provider and
   * the provided street address provider.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
