...
/**
 * Signals that a test class contains an issue awaiting some action and due to the test
 * configuration it should be skipped. This exception is typically thrown when a test encounters a
 * condition that requires manual intervention or external resolution before the test can proceed.
 *
 * <p>This exception extends {@link CRuntimeException} and is used in test scenarios where:
 *
 * <ul>
 *   <li>A test depends on an external service that is temporarily unavailable
 *   <li>A known issue exists that is awaiting a fix
 *   <li>Environmental conditions prevent the test from running reliably
 * </ul>
 *
...
public class CSkipAwaitingTestException extends CRuntimeException {
   ...
   *
   * <p>The description should clearly indicate why the test is being skipped and what action is
   * being awaited to resolve the issue.
   *
   ...
   * }</pre>
   *
   * @param description a detailed message describing the reason for skipping the test and what
