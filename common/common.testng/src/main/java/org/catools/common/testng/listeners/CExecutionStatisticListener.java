 ...
 *
 * <ul>
 *   <li>Real-time tracking of test execution statistics
 *   <li>Thread-safe operations for concurrent test execution
 *   <li>Automatic categorization of tests by execution status
 *   <li>Support for dynamic test removal and statistics updates
 * </ul>
 *
...
public class CExecutionStatisticListener implements ISuiteListener, IResultListener {
   ...
   *
   * <p>This count includes all test methods in all statuses: passed, failed, skipped, running, and
   * waiting. The total is automatically updated whenever test methods are added or removed from
   * tracking.
   *
   * <h3>Usage Example:</h3>
   *
   ...
   */
  @Getter private static int total;

  /**
   * The number of test methods that have passed successfully.
   *
   * <p>This count includes tests with {@link CExecutionStatus#SUCCESS} and {@link
   * CExecutionStatus#SUCCESS_PERCENTAGE_FAILURE} statuses. The count is automatically updated as
   * tests complete.
   *
   * <h3>Usage Example:</h3>
   *
   ...
   */
  @Getter private static int passed;

  /**
   * The number of test methods that have failed.
   *
   * <p>This count includes tests with {@link CExecutionStatus#FAILURE} status. The count is
   * automatically updated as tests complete with failures.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   */
  @Getter private static int failed;

  /**
   * The number of test methods that have been skipped.
   *
   * <p>This count includes tests with {@link CExecutionStatus#SKIP}, {@link
   * CExecutionStatus#IGNORED}, and {@link CExecutionStatus#DEFERRED} statuses. The count is
   * automatically updated when tests are skipped for any reason.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   */
  @Getter private static int skipped;

  /**
   * The number of test methods that are currently running.
   *
   * <p>This count includes tests with {@link CExecutionStatus#WIP} (Work In Progress) status. This
   * provides real-time insight into active test execution.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   */
  @Getter private static int running;

  /**
   * The number of test methods that are waiting to be executed.
   *
   * <p>This count represents tests that have been registered but haven't started execution yet.
   * It's calculated as the difference between total tests and all other status categories (passed +
   ...
   */
  @Getter private static int waiting;
   ...
   *
   * <p>This method provides a way to dynamically remove test methods from the statistics tracking,
   * which can be useful for conditional test execution scenarios or when tests need to be excluded
   * from reporting after registration.
   *
   * <p>The method is thread-safe and will automatically recalculate all statistics after the
   * removal operation.
   *
   * <h3>Usage Examples:</h3>
   *
   ...
   * }</pre>
   *
   * @param method the TestNG method to remove from tracking. The method is identified by the
   *     combination of its test class name and method name.
   ...
   *
   * <p>This method initializes the statistics tracking by clearing any previous data and
   * registering all test methods in the suite with {@link CExecutionStatus#CREATED} status. This
   * provides the foundation for tracking test execution progress.
   *
   * <p>The method processes all methods returned by {@link ISuite#getAllMethods()}, which includes
   * all test methods across all test classes in the suite.
   *
   * <h3>Execution Flow:</h3>
   *
   * <ol>
   *   <li>Clears any existing method tracking data
   *   <li>Iterates through all methods in the suite
   *   <li>Registers each method with CREATED status
   *   <li>Updates all statistics counters
   * </ol>
   *
   * <h3>Usage Example:</h3>
   *
   ...
   *
   * <p>This method updates the execution status of the test method to {@link CExecutionStatus#WIP}
   * (Work In Progress), indicating that the test is currently running. This allows for real-time
   * monitoring of active tests.
   *
   * <p>The statistics are automatically recalculated to reflect the change, incrementing the
   * running counter and decrementing the waiting counter.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>This method updates the execution status of the test method to {@link
   * CExecutionStatus#SUCCESS}, indicating that the test has passed all assertions and completed
   * without errors.
   *
   * <p>The statistics are automatically recalculated to reflect the successful completion,
   * incrementing the passed counter and decrementing the running counter.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>This method updates the execution status of the test method to {@link
   * CExecutionStatus#FAILURE}, indicating that the test has failed due to assertion failures,
   * exceptions, or other errors.
   *
   * <p>The statistics are automatically recalculated to reflect the failure, incrementing the
   * failed counter and decrementing the running counter.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param result the test result containing information about the failed test, including exception
   *     details and execution context
   * @see ITestResult#getMethod()
   ...
   *
   * <p>This method updates the execution status of the test method to {@link
   * CExecutionStatus#SKIP}, indicating that the test was not executed. Tests can be skipped for
   * various reasons including dependency failures, conditional execution logic, or explicit skip
   * annotations.
   *
   * <p>The statistics are automatically recalculated to reflect the skip, incrementing the skipped
   * counter and decrementing the waiting counter.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param result the test result containing information about the skipped test, including
   *     potential skip reasons and execution context
   * @see ITestResult#getMethod()
   * @see ITestResult#getThrowable()
   ...
   *
   * <p>This method is invoked for tests that have a defined success percentage and the failure
   * falls within that acceptable range. Such tests are treated as successful for reporting
   * purposes, so the status is updated to {@link CExecutionStatus#SUCCESS}.
   *
   * <p>This feature is useful for tests that may have intermittent issues but are generally
   * reliable, allowing for some tolerance in test execution results.
   *
   * <h3>Usage Example:</h3>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param result the test result containing information about the test that failed but is within
   *     the acceptable success percentage range
   * @see ITestResult#getMethod()
