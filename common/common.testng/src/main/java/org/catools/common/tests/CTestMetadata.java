 ...
 *
 * <p>CTestMetadata extends CList to provide convenient methods for handling test metadata entries
 * consisting of name-value pairs. This class is particularly useful for storing and querying
 * test-related information such as test tags, categories, properties, or any other metadata
...
public class CTestMetadata extends CList<CTestMetadata.MetaData> {
   ...
   *
   * <p>This method creates a new MetaData instance and adds it to the collection. Duplicate entries
   * (same name and value) are allowed.
   *
   * <p><b>Examples:</b>
   *
   * <pre>{@code
   ...
   *
   * <p>This method uses exact string matching (case-sensitive) to find the metadata entry. Both
   * name and value must match exactly for the method to return true.
   *
   * <p><b>Examples:</b>
   *
   * <pre>{@code
   ...
   *
   * <p>This method is the logical inverse of {@link #has(String, String)}. It uses exact string
   ...
   *
   * <p>This method prevents duplicate metadata entries by checking if the exact name-value pair
   * already exists in the collection before adding it. If the entry already exists, no action is
   * taken.
   *
   * <p><b>Examples:</b>
   *
   ...
   *
   * <p>This inner class encapsulates a metadata entry with a name (key) and value. Both fields can
   * be null. The class uses Lombok annotations for automatic generation of getters, setters,
