...
/**
 * Abstract base class for execution statistic converters used in Log4j pattern layouts. This class
 * provides common functionality for converting execution statistics to formatted strings in log
 * output. Subclasses should implement specific statistic collection logic.
 *
...
public abstract class CBaseExecutionStatisticConverter extends LogEventPatternConverter {
   ...
   *
   * <p>This constructor initializes the converter with a name, style, format string, and a supplier
   * function that provides the statistic values. The string format defaults to "%d" if a blank or
   * null format is provided.
   *
  ...
  /**
   * Validates the provided options array and extracts the first option if available. This utility
   * method is commonly used in converter constructors to handle optional configuration parameters
   * from Log4j pattern layouts.
   *
   * <p>The method ensures that at most one option is provided and returns it, or null if no options
   * are specified.
   *
   * @param options the options array from the Log4j pattern configuration. Can be null, empty, or
  ...
  /**
   * Formats the execution statistic and appends it to the log output. This method is called by the
   * Log4j framework during log event processing to convert the statistic value into its string
   * representation.
   *
   * <p>The method retrieves the current statistic value from the supplier function, formats it
   * according to the configured string format, and appends the result to the provided
   * StringBuilder.
   *
