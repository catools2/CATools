 ...
 *
 * <p>This class provides static methods to access various TestNG configuration parameters from
 * HOCON configuration files. All configuration values are retrieved using the CHocon utility class.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class CTestNGConfigs {
   ...
   *
   * <p>This method loads the class specified in the configuration property {@code
   * catools.testng.base_test_class_loader} and returns its Class object.
   *
   ...
   *
   * <p>This method reads the listener class names from the configuration property {@code
   * catools.testng.listeners} and instantiates them. If a listener class cannot be found or
   * instantiated, a warning message is printed and the listener is skipped.
   *
    ...
    if (CHocon.has(Configs.CATOOLS_TESTNG_LISTENERS)) {
      for (String listener : CHocon.asStrings(Configs.CATOOLS_TESTNG_LISTENERS)) {
        try {
          ...
          System.out.println(
              "Could not find CATOOLS_TESTNG_LISTENERS parameter "
                  + listener
                  + " in the class path.");
        }
      }
   ...
   *
   * <p>The severity level determines which tests should be executed based on their severity rating.
   * Tests with severity levels higher than this value may be skipped.
   *
   ...
   *
   * <p>The regression depth determines how many levels of regression tests should be executed.
   * Higher values indicate more comprehensive regression testing.
   *
  ...
  /**
   * Retrieves the list of annotations that will cause a test to be ignored if ANY of them are
   * present.
   *
   * <p>If a test method or class is annotated with any of the annotations in this list, the test
  ...
  /**
   * Retrieves the list of annotations that will cause a test to be ignored if ALL of them are
   * present.
   *
   * <p>If a test method or class is annotated with all of the annotations in this list, the test
   * will be skipped during execution.
   *
   ...
   *
   * <p>Only tests that are annotated with all of the annotations in this list will be executed.
   * This provides a way to filter tests to run only specific subsets.
   *
   ...
   *
   * <p>Tests that are annotated with any of the annotations in this list will be executed. This
   * provides a way to run tests that have at least one of the specified annotations.
   *
   ...
   *
   * <p>When a test fails, it can be automatically retried up to this many times before being marked
   * as failed. A value of 0 means no retries.
   *
   ...
   *
   * <p>This method returns the package names that TestNG should scan for test classes. Only classes
   * in these packages (and their sub-packages) will be considered for test execution.
   *
   ...
   *
   * <p>This determines how TestNG should execute tests in parallel at the test level. Common values
   * include METHODS, CLASSES, TESTS, or NONE.
   *
  ...
  public static XmlSuite.ParallelMode getTestLevelParallel() {
    return CHocon.asEnum(
        Configs.CATOOLS_TESTNG_TEST_LEVEL_PARALLEL_MODE, XmlSuite.ParallelMode.class);
  }

  /**
   * Retrieves the thread count for test level parallel execution.
   *
   * <p>This specifies how many threads should be used when executing tests in parallel at the test
   * level. This works in conjunction with the test level parallel mode.
   *
   ...
   *
   * <p>This determines how TestNG should execute test suites in parallel at the suite level. This
   * is typically used when running multiple test suites concurrently.
   *
  ...
  public static XmlSuite.ParallelMode getSuiteLevelParallel() {
    return CHocon.asEnum(
        Configs.CATOOLS_TESTNG_SUITE_LEVEL_PARALLEL_MODE, XmlSuite.ParallelMode.class);
  }

  /**
   * Retrieves the thread count for suite level parallel execution.
   *
   * <p>This specifies how many threads should be used when executing test suites in parallel at the
   * suite level. This works in conjunction with the suite level parallel mode.
   *
   ...
   * implementation or resolution), the entire class will be skipped during test execution.
   *
   ...
   * {@code @Ignore} annotation), the entire class will be skipped during test execution.
   *
   ...
   *
   * <p>This specifies the filename that will be used for the TestNG results XML output. The XML
   * file contains detailed information about test execution results.
   *
  ...
  private enum Configs implements CHoconPath {
    ...
    CATOOLS_TESTNG_IGNORE_TEST_WITH_ANY_ANNOTATION(
        "catools.testng.ignore_test_with_any_annotation"),
    CATOOLS_TESTNG_IGNORE_TEST_WITH_ALL_ANNOTATION(
        "catools.testng.ignore_test_with_all_annotation"),
