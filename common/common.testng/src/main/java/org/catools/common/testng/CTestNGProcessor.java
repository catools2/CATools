...
/**
 * A utility class for processing TestNG test suites, providing methods to build, configure, and
 * execute TestNG suites programmatically.
 *
 * <p>This class supports various ways of running TestNG tests:
 *
 * <ul>
 *   <li>Building test suites from class collections
 *   <li>Processing XML suite files (local or from resources)
 *   <li>Running tests by test IDs or class names
 *   <li>Executing XML suites with custom configurations
 * </ul>
 *
 * @author CATools Team
 * @since 1.0
...
public class CTestNGProcessor {
   ...
   *
   * <p>This method creates an XML suite file that can be used to run TestNG tests. The test classes
   * are organized into groups, and each group becomes a test within the suite.
   *
   * @param testClassesMap a map where keys are test group names and values are sets of test class
   *     names
   ...
   *
   * <p>This method provides a convenient way to create a TestNG suite from a simple set of test
   * classes. If no group mapper is provided, all classes are placed in a single test group named
   * "Test".
   *
   * @param testClasses a set of fully qualified test class names
   * @param filename the path where the XML suite file will be created
   * @param testClassGroupMapper an optional function to group test classes; if null, all classes go
   *     to "Test" group
   * @param xmlSuiteAdjuster an optional consumer to customize the XmlSuite before writing to file
   * @return a CFile object representing the created XML suite file
   * @example
   ...
   *
   * <p>This method loads a TestNG XML suite file from the local filesystem and executes it. The
   * file must exist and have a .xml extension.
   *
   ...
   *
   * <p>This method loads a TestNG XML suite file from the classpath resources, copies it to a
   * temporary location, and executes it. The resource must exist and have a .xml extension.
   *
   ...
   * and executes it. Test IDs are typically issue keys or identifiers that map to specific test
   * classes.
   *
   ...
   * @param filterTestsWhichWillSkipInRun if true, filters out tests that would be skipped during
   *     execution
   * @return the TestNG execution status code (0 for success, non-zero for failure)
   * @example
   *     <pre>{@code
   ...
   * }</pre>
   */
  public static int processTestByTestIds(
   ...
   *
   * <p>This method creates a TestNG suite from the provided test class names and executes it. This
   * is useful when you know exactly which test classes you want to run.
   *
   ...
   * }</pre>
   */
  public static int processTestByClassNames(
   ...
   * }</pre>
   */
  public static int processTestClasses(
      CSet<String> testClasses,
      String filename,
      Function<CSet<String>, Map<String, CSet<String>>> testClassGroupMapper,
      Consumer<XmlSuite> xmlSuiteAdjuster) {
   ...
   *
   * <p>This method takes already constructed XmlSuite objects and executes them using TestNG. It
   * applies any configured listeners from CTestNGConfigs and saves the suite configuration to an
