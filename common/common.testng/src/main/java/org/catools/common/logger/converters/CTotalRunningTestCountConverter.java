...
/**
 * A Log4j pattern converter that displays the total count of currently running tests. This
 * converter integrates with the TestNG execution framework to provide real-time statistics about
 * test execution status in log output.
 *
 * <p>The converter tracks tests that are currently in the RUNNING state as monitored by {@link
 * CExecutionStatisticListener}. This is useful for monitoring concurrent test execution and
 * understanding how many tests are actively running at any given moment.
 *
 * <p>The converter is registered with Log4j using the key "ETR" (Execution Tests Running) and can
 * be used in pattern layouts to display running test counts in log messages.
 *
 ...
 *
 * <p>This converter relies on {@link CExecutionStatisticListener} being properly registered as a
 * TestNG listener. The listener tracks test method execution states and provides the running count
 * through its static {@code getRunning()} method.
 *
 * <h3>Thread Safety:</h3>
 *
 * <p>This converter is thread-safe as it delegates to the thread-safe {@link
 * CExecutionStatisticListener#getRunning()} method for retrieving the current running test count.
 *
...
public class CTotalRunningTestCountConverter extends CBaseExecutionStatisticConverter {
   ...
   *
   * <p>This constructor initializes the converter to track and display the count of currently
   * running tests. The running count is obtained from the execution statistic listener which
   * monitors TestNG test execution states.
   *
   * <p>The string format parameter allows customization of how the running count is displayed in
   * log output. If null or blank, defaults to "%d" for plain numeric display.
   *
   * @param stringFormat the format string for displaying the running test count. Examples: "%d"
   *     (plain number), "%03d" (zero-padded to 3 digits), "Running: %d" (with descriptive text). If
   *     null or blank, defaults to "%d"
   *     <h3>Examples:</h3>
   *     <pre>{@code
  ...
  protected CTotalRunningTestCountConverter(final String stringFormat) {
    super("Total Running", "ETR", stringFormat, () -> CExecutionStatisticListener.getRunning());
  }

  /**
   * Creates a new instance of CTotalRunningTestCountConverter with configuration options.
   *
   * <p>This factory method is called by the Log4j framework during pattern layout initialization.
   * It processes the configuration options from the pattern layout and creates a properly
   * configured converter instance.
   *
   * <p>This method validates that at most one option is provided (the format string) and delegates
   * to {@link CBaseExecutionStatisticConverter#validateAndGetOption(String[])} for option
   * processing.
   *
   * @param options configuration options from the Log4j pattern layout. Should contain at most one
   *     element representing the format string. Can be null, empty array, or single-element array
   *     with format string
