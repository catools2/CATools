...
public class CHoconConfigTest {
  ...
  public void testVaultFallback() throws Exception {
    ...
    CVaultClient fakeClient = new CVaultClient(null, "fake") {
      ...
      public String getValue(String key, String defaultTo) {
        ...
        if (key.equals(path.toUpperCase().replaceAll("\\.", "_")))
          return "vault-secret-value";
        return defaultTo;
      }
    ...
    CHoconConfig cfg = new CHoconConfig(
        ConfigFactory.defaultApplication().withValue("catools.vault.enabled", ConfigValueFactory.fromAnyRef(true)),
        path);
    String value = cfg.asString();
  ...
  public void testVaultPathNotPresent() throws Exception {
    ...
    String value = cfg.asString();
    Assert.assertEquals(
        value, CStringUtil.EMPTY, "Expected empty string when vault does not contain the path");
  }
    ...
    CVaultClient fakeClient = new CVaultClient(null, "fake") {
      ...
      public String getValue(String key, String defaultTo) {
        if (key == null)
          return defaultTo;
        if (key.equals(path))
          return CStringUtil.EMPTY; // path exists but value is empty
        return defaultTo;
      }
