...
/**
 * A specialized collection class for managing {@link CRandomState} objects. This class extends
 * {@link HashSet} to provide a type-safe container for states, ensuring unique state entries based
 * on the default {@code equals} and {@code hashCode} implementations of {@link CRandomState}.
 *
 * <p>This class is particularly useful in faker libraries or data generation scenarios where you
 * need to manage a collection of states (e.g., US states, provinces, etc.) and ensure no duplicates
 * are present.
 *
...
public class CRandomStates extends HashSet<CRandomState> {
   ...
   *
   * <p>The backing {@link HashSet} is initialized with default initial capacity (16) and load
   * factor (0.75), providing efficient performance for most use cases.
   *
   * <h4>Example:</h4>
   *
   * <pre>{@code
