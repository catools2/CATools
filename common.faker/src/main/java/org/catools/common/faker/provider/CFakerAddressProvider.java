 ...
 * 
 * <p>This class provides functionality to generate complete random addresses
 * including country, state, city, and street information. It can generate
 * addresses for random locations or specific states and cities.</p>
 * 
 * @since 1.0
 */
@AllArgsConstructor
public class CFakerAddressProvider {
   ...
   * 
   * <p>This method selects a random state from the country's available states
   * and then generates a complete address including street information for
   * that location.</p>
   * 
   ...
   * 
   * <p>This method generates a complete address for the specified state by
   * randomly selecting a city within that state and generating street information.
   * If the provided state code is not found, returns null.</p>
   * 
   ...
   * 
   * <p>This method generates a complete address for the specified state and city
   * combination. It validates that both the state code and city name exist in the
   * country's data before generating the address with random street information.</p>
   * 
   ...
   * @return a randomly generated address for the specified state and city, 
   *         or null if either state or city is not found
   * 
   * @example
   * <pre>
   * CFakerAddressProvider addressProvider = new CFakerAddressProvider(countryProvider, streetProvider);
