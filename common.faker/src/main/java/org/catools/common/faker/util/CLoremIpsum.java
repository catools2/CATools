 ...
 * <p>
 * This class provides methods to generate random text at different levels: words, statements,
 * and paragraphs. Each level can be customized with length ranges and other parameters.
 * The generated text uses lowercase letters and digits, with proper capitalization and
 * punctuation applied automatically.
 * </p>
 *
 * <p>Example usage:</p>
 * <pre>{@code
...
public class CLoremIpsum {
  ...
  /**
   * Generates a single paragraph with customizable word, statement, and paragraph length parameters.
   * <p>
   * The paragraph is constructed by combining multiple statements until the target paragraph length
   * is reached. Each statement starts with a capital letter and ends with a random punctuation mark.
   * The final paragraph is trimmed to the exact specified length and ends with a special character.
   * </p>
   *
   ...
   * @throws CInvalidRangeException if any minimum value exceeds its corresponding maximum value,
   ...
   * <p>
   * Each paragraph is generated independently using the same length parameters,
   * and all paragraphs are joined together with spaces.
   * </p>
   *
   * @param minWordLength      minimum length for each word (must be non-negative)
   * @param maxWordLength      maximum length for each word (must be >= minWordLength)
   ...
   * <p>
   * A statement is constructed by combining multiple words until the target statement length
   * is reached. The statement starts with a random capital letter and ends with a random
   * punctuation mark (from the set: !?,;......). The final statement is trimmed to the
   ...
   * <p>
   * Each statement is generated independently using the same length parameters,
   * and all statements are joined together with spaces.
   * </p>
   *
   * @param minWordLength      minimum length for each word (must be non-negative)
   * @param maxWordLength      maximum length for each word (must be >= minWordLength)
   ...
   * <p>
   * The word consists of lowercase letters and digits, randomly selected from the
   * character range 'a' to 'z'. The actual length is randomly chosen between
   ...
   * <p>
   * Each word is generated independently using the same length parameters,
   * and all words are joined together with spaces.
   * </p>
   *
   * @param minWordLength minimum length for each word (must be non-negative)
   * @param maxWordLength maximum length for each word (must be >= minWordLength)
