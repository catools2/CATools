 ...
 * This exception is typically thrown when attempting to generate fake data for a country that is
 * not recognized by the ISO 3166-1 alpha-3 standard or is not currently supported by the faker
 * library.
 *
 * <p>The exception message includes the invalid country code that was provided and a list of all
 * currently supported country codes for reference.
 *
...
public class CFakerCountryNotFoundException extends RuntimeException {
  ...
  /**
   * Constructs a new CFakerCountryNotFoundException with a detailed message indicating the invalid
   * country code and listing all supported country codes.
   *
   * <p>The exception message follows the format: "Country code {countryCode3} is not a valid ISO3
   * country code or it is not support at the moment of time. Supporting Countries: [list of
   * supported codes]"
   *
   * <h3>Usage Examples:</h3>
   *
   ...
   * }</pre>
   *
   * @param countryCode3 the invalid ISO 3166-1 alpha-3 country code that was provided. This should
   *     be a 3-character string representing the country code that failed validation. Must not be
   *     null.
