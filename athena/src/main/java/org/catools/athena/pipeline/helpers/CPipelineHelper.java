...
public class CPipelineHelper {
  ...
  public static PipelineExecutionDto buildExecutionDto(
      ITestResult testResult,
      PipelineDto pipeline,
      Instant beforeClassStartTime,
      Instant beforeMethodStartTime,
    ...
    executionDto.setMetadata(
        CPipelineHelper.getExecutionMetadata(testResult, beforeClassStartTime, Instant.now()));
  ...
  public static Set<MetadataDto> getExecutionMetadata(
      ITestResult testNgResult, Instant testStartTime, Instant testEndTime) {
    CTestResult testResult =
        new CTestResult(testNgResult, Date.from(testStartTime), Date.from(testEndTime));
    ...
    if (testResult.getOpenDefectIds() != null && testResult.getOpenDefectIds().isNotEmpty()) {
      testResult
          .getOpenDefectIds()
          .forEach(t -> executionMetaData.add(new MetadataDto("Open Defect ID", t)));
    }

    if (testResult.getDefectIds() != null && testResult.getDefectIds().isNotEmpty()) {
      testResult
          .getDefectIds()
          .forEach(t -> executionMetaData.add(new MetadataDto("Defect ID", t)));
    }

    if (testResult.getDeferredId() != null && testResult.getDeferredId().isNotEmpty()) {
      testResult
          .getDeferredId()
          .forEach(t -> executionMetaData.add(new MetadataDto("Deferred ID", t)));
    }

    if (testResult.getMethodGroups() != null && testResult.getMethodGroups().isNotEmpty()) {
      testResult
          .getMethodGroups()
          .forEach(t -> executionMetaData.add(new MetadataDto("Method Group", t)));
    }

    if (testResult.getParameters() != null && testResult.getParameters().length > 0) {
      executionMetaData.add(
          new MetadataDto("Parameters", CJsonUtil.toString(testResult.getParameters())));
    }
