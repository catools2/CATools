 ...
 * their values as JSON arrays containing "customFieldOption" objects. It handles the parsing of
 * complex custom field structures by removing unnecessary metadata fields and extracting the
 * meaningful option values.
 *
 * <p>The parser is designed to handle custom fields such as:
 *
 * <ul>
 *   <li>Single-select dropdown fields
 *   <li>Multi-select dropdown fields
 *   <li>Radio button fields
 *   <li>Checkbox fields
 * </ul>
 *
...
public class CEtlJiraCustomFieldOptionParser implements CEtlJiraFieldParser {
   ...
   *
   * <p>The parser will analyze the field to determine if it contains custom field options and
   * extract meaningful values while filtering out metadata.
   *
   * <p>Example usage:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param field the Jira issue field containing custom field options to be parsed. Must not be
   *     null and should contain a JSONArray value for proper parsing.
   */
   ...
   *
   * <p>Lower values indicate higher priority. This parser has the highest priority (rank 0) for
   * custom field option parsing, ensuring it's evaluated before other field parsers when dealing
   * with JSONArray values containing "customFieldOption".
   *
   ...
   *
   * <ol>
   *   <li>The value is a JSONArray instance
   *   <li>The array content contains "customFieldOption" (case-insensitive)
   * </ol>
   *
   ...
   *
   * <p>This method processes the JSON array by:
   *
   * <ol>
   *   <li>Parsing the JSONArray into a list of LinkedHashMap objects
   *   <li>Removing metadata fields ("self" and "id") that are not meaningful for reporting
   *   <li>Extracting the actual option values
   *   <li>Filtering out default/placeholder values using {@link #isDefaultValue(String)}
   *   <li>Mapping the field name to the extracted value(s)
   * </ol>
   *
  ...
  public CHashMap<String, String> getNameValuePairs() {
    ...
    try {
      CList<LinkedHashMap<String, String>> parse =
          CJsonUtil.read(field.getValue().toString(), CList.class);
