 ...
 * @see CList
 */
public interface CMapVerify<K, V>
    extends CBaseMapExtension<K, V>, CObjectVerify<Map<K, V>, CMapState<K, V>> {
  ...
  default void verifyContains(
      K expectedKey, V expectedValue, final String message, final Object... params) {
   ...
   */
  default void verifyContains(
      Map.Entry<K, V> expected, final String message, final Object... params) {
    _verify(
        expected,
        (a, e) -> {
          ...
          if (!_get().containsKey(expected.getKey())) {
            logger.warn(
                "Key does not exist in map. Expected: '{}' Actual: '{}'",
                expected.getKey(),
                _get().keySet());
            return false;
          }
  ...
  default void verifyContainsNone(
      Map<K, V> expected, final String message, final Object... params) {
    _verify(
        expected,
        (a, e) -> {
          if (expected == null) {
  ...
  default void verifyContainsNone(Map<K, V> expected) {
    verifyContainsNone(
        expected, getDefaultMessage(("Not Contains Any Record From The Expected Map")));
  }
  ...
  default void verifyEmptyOrContains(
    ...
    _verify(
        ...
        (a, e) -> {
          ...
          if (!_get().containsKey(expected.getKey())) {
            logger.warn(
                "Key does not exist in map. Expected Key: '{}' Actual keys: '{}'",
                expected.getKey(),
                _get().keySet());
            return false;
          }
  ...
  default void verifyEmptyOrNotContains(Map.Entry<K, V> expected) {
    verifyEmptyOrNotContains(
        expected, getDefaultMessage(("Is Empty Or Not Contains The Expected Value")));
  }
   ...
   * note that actual map might have some entries but the point is to ensure that not all expected
  ...
  default void verifySizeIsGreaterThanOrEqual(int expected) {
    verifySizeIsGreaterThanOrEqual(
        expected, getDefaultMessage(("Size Is Greater Than Expected Value")));
  }
  ...
  default void verifySizeIsGreaterThanOrEqual(
      int expected, final String message, final Object... params) {
  ...
  default void verifySizeIsLessThanOrEqual(int expected) {
    verifySizeIsLessThanOrEqual(
        expected, getDefaultMessage(("Size Is Less Than Or Equal Expected Value")));
  }
