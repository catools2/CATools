...
public interface CCollection<E>
    extends CIterable<E>, Collection<E>, CCollectionVerifier<E>, CCollectionState<E> {
  ...
  default E getAnyAndRemove() {
    ...
    Object[] objects = toArray();
    Object object = objects[new SecureRandom().nextInt(objects.length)];
    remove(object);
    return (E) object;
  }
  ...
  default CList<CList<E>> partition(int size) {
    return new CList<>(
        Lists.partition(new ArrayList<>(get()), size).stream()
            .map(CList::new)
