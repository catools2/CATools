...
/**
 * WebKit Playwright provider supporting headed and headless modes.
 * <p>
 ...
 * <p>Example usage:</p>
 * <pre>{@code
 * CWebKitPlaywrightProvider provider = new CWebKitPlaywrightProvider();
 ...
 */
@Slf4j
public class CWebKitPlaywrightProvider implements CPlaywrightProvider {
  ...
  private final ThreadLocal<Browser> browserThreadLocal = ThreadLocal.withInitial(() -> {
    ...
    log.info("WebKit browser initialized for thread: {}. Launch Options: {}",
             Thread.currentThread().getName(), launchOptions);

    return browser;
  });

  protected CWebKitPlaywrightProvider() {
