...
public class CEtlKubeCacheManager {
  ...
  private static final CMap<String, CEtlKubeContainerMetaData> CONTAINER_METADATA =
      new CHashMap<>();

  /**
   * Get the project from DB if exists otherwise create one and save it in local cash for farther
   * usage
   *
   * @param name
   * @return
  ...
  /**
   * Get the status from DB if exists otherwise create one and save it in local cash for farther
   * usage
   *
   * @param status
   * @param phase
   ...
   * @return
   */
  public static CEtlKubePodStatus getStatus(
      String status, String phase, String message, String reason) {
    return read(
        STATUSES,
        status + phase + message + reason,
        () -> {
          CEtlKubePodStatus result =
              CEtlKubePodStatusDao.getEtlKubePodStatus(status, phase, message, reason);
          if (result != null) {
            return result;
          }
  ...
  /**
   * Get the pod metadata from DB if exists otherwise create one and save it in local cash for
   * farther usage
   *
   * @param type
   * @param name
  ...
  /**
   * Get the container from DB if exists otherwise create one and save it in local cash for farther
   * usage
   *
   * @param name
   * @param value
  ...
  public static CEtlKubeContainerMetaData getContainerMetadata(String name, String value) {
    return read(
        ...
        name + value,
        () -> {
          CEtlKubeContainerMetaData result =
              CEtlKubeContainerMetaDataDao.getKubeContainerMetaData(name, value);
          if (result != null) {
            return result;
          }
