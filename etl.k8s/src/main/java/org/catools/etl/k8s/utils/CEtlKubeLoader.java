...
public class CEtlKubeLoader {
  ...
  public static CEtlKubePod translatePod(String projectName, CKubePod pod, CDate loadTime) {
    ...
    CEtlKubePodStatus status =
        CEtlKubeCacheManager.getStatus(
            pod.getStatus().getStatus(),
    ...
    pod.getAnnotations()
        .forEach(
            (k, v) -> kubePod.addMetaData(CEtlKubeCacheManager.getPodMetadata("ANNOTATION", k, v)));
    pod.getLabels()
        .forEach((k, v) -> kubePod.addMetaData(CEtlKubeCacheManager.getPodMetadata("LABEL", k, v)));
    pod.getMetadata()
        .forEach(
            (k, v) -> kubePod.addMetaData(CEtlKubeCacheManager.getPodMetadata("METADATA", k, v)));

    pod.getContainers()
        .forEach(c -> kubePod.addContainer(translateContainer("eternal", c, loadTime)));
    pod.getEphemeralContainers()
        .forEach(c -> kubePod.addContainer(translateContainer("ephemeral", c, loadTime)));
    pod.getInitContainers()
        .forEach(c -> kubePod.addContainer(translateContainer("init", c, loadTime)));

    return kubePod;
  }

  private static CEtlKubeContainer translateContainer(
      String type, CKubeContainer c, CDate loadTime) {
    CEtlKubeContainer kubeContainer =
        new CEtlKubeContainer(
            type,
            c.getName(),
            c.getImage(),
            c.getImageId(),
    ...
    if (terminatedState != null) {
      if (StringUtils.isNotBlank(terminatedState.getMessage()))
        kubeContainer.addMetaData(
            CEtlKubeCacheManager.getContainerMetadata(
                "terminated_message", terminatedState.getMessage()));

      if (StringUtils.isNotBlank(terminatedState.getReason()))
        kubeContainer.addMetaData(
            CEtlKubeCacheManager.getContainerMetadata(
                "terminated_reason", terminatedState.getReason()));
    }
    ...
    if (waitingState != null) {
      if (StringUtils.isNotBlank(waitingState.getMessage()))
        kubeContainer.addMetaData(
            CEtlKubeCacheManager.getContainerMetadata(
                "waiting_message", waitingState.getMessage()));

      if (StringUtils.isNotBlank(waitingState.getReason()))
        kubeContainer.addMetaData(
            CEtlKubeCacheManager.getContainerMetadata("waiting_reason", waitingState.getReason()));
    }
