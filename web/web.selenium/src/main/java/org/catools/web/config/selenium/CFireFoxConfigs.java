 ...
 *
 * <p>This class provides access to Firefox-specific configuration properties including binary path,
 * browser arguments, page load strategy, and headless mode settings. All configuration values are
 * retrieved from the HOCON configuration system.
 *
...
public class CFireFoxConfigs {
   ...
   *
   * <p>This method returns the configured path to the Firefox browser executable. If not specified
   * in configuration, the system default Firefox installation will be used.
   *
   * <h3>Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>These arguments are applied to Firefox when starting the browser instance. Common arguments
   * include preferences for privacy, security, and performance settings.
   *
   * <h3>Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>The page load strategy determines how WebDriver waits for pages to load. This affects the
   * timing of when WebDriver considers a page "ready" for interaction.
   *
   * <h3>Available Strategies:</h3>
   *
   * <ul>
   *   <li>{@code NORMAL} - Wait for all resources to load (default)
   *   <li>{@code EAGER} - Wait for DOM to be ready, but not all resources
   *   <li>{@code NONE} - Return immediately after initial page load
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   *
   * <p>Headless mode runs Firefox without a graphical user interface, which is useful for automated
   * testing, CI/CD pipelines, and server environments where no display is available.
   *
   * <h3>Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>These arguments are applied to Firefox when running in headless mode, in addition to the
   * default arguments. They typically include optimizations and configurations specific to headless
   * operation.
   *
   * <h3>Example:</h3>
   *
