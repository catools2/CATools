...
/**
 * Utility class for interacting with Selenium Grid infrastructure. Provides methods to retrieve
 * node information and copy files to remote grid nodes.
 *
 * <p>This class is designed to work with Selenium Grid setups where tests run on remote nodes. It
 * helps with common tasks like getting node connection details and file transfers.
 *
 * @author catools
 * @since 1.0.0
 */
...
public class CGridUtil {
   ...
   *
   * <p>This method queries the Selenium Grid Hub to find which node is handling the specified
   * session and returns the connection details.
   *
   * @param sessionId the Selenium Page session ID to look up
   * @return a String array where index 0 is the hostname and index 1 is the port, or null if remote
    ...
    try {
      String targetURL =
          String.format(
              ...
              CGridConfigs.getGridHubIP(), CGridConfigs.getGridHubPort(), sessionId);
   ...
   *
   * <p>This is a convenience method that copies the source file to the remote node while preserving
   * the original filename and directory structure.
   *
   ...
   * @throws RuntimeException if the file copy operation fails or node information cannot be
   *     retrieved
   * @example
   *     <pre>{@code
   ...
   *
   * <p>This method allows you to specify both the source file and the destination folder on the
   * remote node, providing more control over where the file is placed.
   *
   * @param sessionId the Selenium Page session ID to identify the target node
   * @param srcFile the source file to be copied to the remote node
  ...
  public static String copyFileToNode(String sessionId, File srcFile, File destFolder) {
    return CFileUtil.copyToRemoteFolder(
        srcFile, Objects.requireNonNull(getHostNameAndPort(sessionId))[0], destFolder);
  }
}
