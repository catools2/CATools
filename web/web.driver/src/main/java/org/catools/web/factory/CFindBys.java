 ...
 * This annotation can be applied to fields or types to specify how to locate elements on a web
 * page.
 *
 * <p>Usage examples:
 *
...
public @interface CFindBys {
  ...
  /**
   * Specifies the XPath expression used to locate the web element(s). This is a required parameter
   * that defines how to find the element on the page.
   *
   * <p>Examples of valid XPath expressions:
   *
   * <ul>
   *   <li>{@code "//button[@id='login']"} - Find button with specific ID
   *   <li>{@code "//div[contains(@class, 'error')]"} - Find div containing error class
   *   <li>{@code "//input[@type='text' and @name='username']"} - Find input with type and name
   *   <li>{@code "//tr[position()>1]/td[2]"} - Find second column of all rows except header
   * </ul>
   *
  ...
  /**
   * Optional name identifier for the element(s) found by this annotation. This can be used for
   * logging, debugging, or referencing purposes. If not specified, defaults to an empty string.
   *
   * <p>Examples:
   *
   * <pre>
  ...
  /**
   * Specifies the maximum time in seconds to wait for the first element to appear. This is useful
   * when dealing with dynamic content that may take time to load. A value of -1 (default) means no
   * explicit wait time is set.
   *
   * <p>Usage scenarios:
   *
   * <ul>
   *   <li>Waiting for AJAX content to load
   *   <li>Waiting for page transitions
   *   <li>Waiting for dynamic elements to appear
   * </ul>
   *
   * <p>Examples:
   *
   ...
   * This is particularly useful when dealing with lists or collections of elements that may load
   * progressively. A value of -1 (default) means no explicit wait time is set.
   *
   * <p>Common use cases:
   *
   * <ul>
   *   <li>Waiting for all items in a dynamic list to load
   *   <li>Progressive loading of table rows
   *   <li>Incremental content loading (like infinite scroll)
   * </ul>
   *
   * <p>Examples:
   *
