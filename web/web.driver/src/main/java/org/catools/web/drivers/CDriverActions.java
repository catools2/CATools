...
/**
 * CDriverActions provides a comprehensive set of web driver action methods for interacting with web
 * elements. This interface extends CDriverWaiter and offers a fluent API for performing various
 * user actions such as clicking, typing, scrolling, and managing cookies. All methods support
...
public interface CDriverActions extends CDriverWaiter {
  ...
  /**
   * Clicks an element after scrolling it into view. This is the standard click method that handles
   * most common clicking scenarios.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Performs a mouse click on an element using the default timeout. This method moves to the
   * element and then performs a mouse click action.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Performs a mouse click on an element with a specified wait time. This method moves to the
   * element and then performs a mouse click action.
   *
   * <p>Example:
   *
  ...
  default boolean scrollLeft(String locator, int scrollSize, int waitSec) {
    return waitUntil(
        ...
        engine ->
            executeScript(
                "document.querySelector('%s').scrollLeft-=arguments[0];".formatted(locator),
                scrollSize));
  }
  ...
  /**
   * Presses multiple keys on the current page.
   *
   * <p>Example:
   *
   * <pre>{@code
   * driver.press(Keys.CONTROL, Keys.ALT, Keys.DELETE);
   * }</pre>
   *
   * @param keysToSend the keys to press
   * @return this instance for method chaining
   */
  default boolean press(CKeys... keysToSend) {
    return performActionOnEngine(
        "Send Keys",
        engine -> {
          engine.press(keysToSend);
          return true;
        });
  }

  /**
   * Presses multiple keys on a specific element.
   *
   * <p>Example:
   *
   * <pre>{@code
   * driver.press("#input-field", Keys.SHIFT, Keys.TAB);
   * }</pre>
   *
   * @param locator the locator of the element to send the keys to
   * @param key the keys to press
   * @return this instance for method chaining
   */
  default boolean press(String locator, CKeys... key) {
    return performActionOnEngine("Press Key" + key, engine -> engine.press(locator, key));
  }

  /**
   * Presses multiple keys on a specific element with a wait time.
   *
   * <p>Example:
   *
   * <pre>{@code
   * driver.press("#input-field", 3, Keys.SHIFT, Keys.TAB);
   * }</pre>
   *
   * @param locator the locator of the element to send the keys to
   * @param waitSec maximum time in seconds to wait for the element
   ...
   * @return this instance for method chaining
   */
  default boolean press(String locator, int waitSec, CKeys... keysToSend) {
    return waitUntil("Send Keys", waitSec, engine -> engine.press(locator, keysToSend));
  }
   ...
   */
  default CDriverCookie getCookie(String name) {
   ...
   */
  default List<CDriverCookie> getCookies() {
   ...
   */
  default CDriverCookie addCookie(String name, String value) {
    return addCookie(new CDriverCookie(name, value));
  }
   ...
   * @return the Cookie object that was added
   */
  default CDriverCookie addCookie(CDriverCookie cookie) {
    return performActionOnEngine("Add Cookie", engine -> engine.addCookie(cookie));
  }

  /**
   * Sets the caret color for all input elements on the engine. This is useful for improving
   * visibility during automated testing.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  default <R> R executeScript(String script, Object... args) {
    return performActionOnEngine(
        "Execute Script", engine -> engine.executeScriptOnDriver(script, args));
  }
  ...
  default <R> R executeScript(
      ...
      @CMcpToolParam(
          ...
          String script) {
    return waitUntil(
        "Execute Script", waitSec, engine -> (R) engine.executeScript(locator, script));
  }

  /**
   * Gets the location of an element on the page after waiting for it.
   *
   * <p>Example:
   *
   * <pre>{@code
   * Point location = driver.getLocation(By.id("logo"), 5);
   * System.out.println("Logo is at: " + location);
   * }</pre>
   *
   * @param locator the locator to find the element
   * @param waitSec maximum time in seconds to wait for the element
   * @return the Point representing the element's location
   */
  default Point getLocation(String locator, int waitSec) {
    return waitUntil("Get Location", waitSec, engine -> engine.getLocation(locator));
  }
  ...
  default <R> R executeAsyncScript(String script, Object... args) {
    return performActionOnEngine(
        "Execute Async Script", engine -> engine.executeAsyncScript(script, args));
  }
  ...
  default <R> R executeAsyncScript(String locator, int waitSec, String script, Object... args) {
    return waitUntil(
        ...
        waitSec,
        engine -> (R) engine.executeAsyncScript(locator, script, args));
  }
  ...
  /**
   * Performs a drag and drop operation by clicking and holding on an element, then moving by the
   * specified offset and releasing.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * @return the WebElement that was dragged
   */
  default boolean dragAndDropTo(final String locator) {
    return dragAndDropTo(locator, 0, 0);
  }
   ...
   * @return the WebElement that was dragged
   */
  default boolean dragAndDropTo(final String locator, int xOffset1, int yOffset1) {
    return dragAndDropTo(locator, xOffset1, yOffset1, 0, 0);
  }
  ...
  default boolean dragAndDropTo(
    ...
    return dragAndDropTo(locator, xOffset1, yOffset1, xOffset2, yOffset2, DEFAULT_TIMEOUT);
  }
   ...
   */
  default boolean dragAndDropTo(
      final String locator, int xOffset1, int yOffset1, int xOffset2, int yOffset2, int waitSec) {
    return waitUntil(
        "Drag And Drop To",
        waitSec,
        driver -> driver.dragAndDropTo(locator, xOffset1, yOffset1, xOffset2, yOffset2));
  }
   ...
   */
  default boolean dragAndDropTo(
      final String source, final String target, int xOffset1, int yOffset1) {
    return dragAndDropTo(source, target, xOffset1, yOffset1, DEFAULT_TIMEOUT);
  }
   ...
   */
  default boolean dragAndDropTo(
      final String source, final String target, int xOffset1, int yOffset1, int waitSec) {
    return waitUntil(
        "Drag And Drop To",
        waitSec,
        driver -> driver.dragAndDropTo(source, target, xOffset1, yOffset1));
  }
  ...
  default boolean moveToElement(final String locator, int xOffset, int yOffset, int waitSec) {
    return waitUntil(
        "Move To Element", waitSec, driver -> driver.moveToElement(locator, xOffset, yOffset));
  }
}
