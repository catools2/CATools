 ...
 *
 * <p>Responsibilities: - Load schemas from "schemas/entities". - Walk schema
...
@Slf4j
@UtilityClass
public class McpStorageConvertor {
    ...
    public JsonNode readSchema(String resourcePath) {
        ...
        String schemaFile = ResourceUtil.readResourceAsString(resource);
    ...
    /**
     * Resolve inheritance by iterating through allOf property and parse each individually and add them to child node
     *
     * @param schema
     * @param child
     * @return
    ...
    private JsonNode resolveInheritance(JsonNode schema, JsonNode child) {
        ...
        if (schema.has("allOf")) {
            schema.get("allOf").elements().forEachRemaining(entry -> {
    ...
    private static JsonNode getRef(String ref) {
        ...
        if (!seenRefs.containsKey(ref)) {
            seenRefs.put(ref, McpStorageConvertor.processSchema(ref));
        }
        ...
        if (mainNode.isObject() && updateNode.isObject()) {
            updateNode.properties().forEach(entry -> {
                ...
                if (existingValue != null && existingValue.isObject() && valueToUpdate.isObject()) {
                    ((ObjectNode) mainNode).set(fieldName, mergeObjects(existingValue, valueToUpdate));
                } else {
