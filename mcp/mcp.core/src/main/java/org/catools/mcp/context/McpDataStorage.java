 ...
 * <p>
 * Use setSharedBetweenThreads(true) to switch to shared/global mode when multiple threads
 * need to access the same stored data (for example, when running an Mcp server).
 * <p>
 * Note: This class stores McpStorageRecord objects in the maps. The convenience write/read
 * overloads accept different value forms but the underlying storage entries are McpStorageRecord instances.
 * <p>
 * A shutdown hook is registered to clear storage at JVM shutdown to reduce the risk of leaking
 * resources across test runs or process lifecycle.
 */
public class McpDataStorage {
    private static final AtomicBoolean sharedBetweenThreads = new AtomicBoolean(false);

    private static final HashMap<String, McpStorageRecord> storage = new HashMap<>();

    private static final ThreadLocal<HashMap<String, McpStorageRecord>> threadLocalData = new ThreadLocal<>();

    static {
        Runtime.getRuntime().addShutdownHook(new Thread(McpDataStorage::clear));
    }

    /**
     * Enable or disable sharing of the storage map between threads.
     *
     * @param enabled true to use a single shared/global map for all threads; false to use thread-local maps
     */
     ...
     *
     * @param clazz String identifier for the schema/class associated with the value; may be used by consumers
    ...
    public static void write(String clazz, String key, JsonNode value) {
        getMap().put(key, new McpStorageRecord(clazz, key, value));
    }

    /**
     * Store a value associated with the given key. This overload accepts a JSON schema node to build the record.
     *
    ...
    public static void write(JsonNode schema, String key, JsonNode value) {
        getMap().put(key, McpStorageRecord.of(schema, key, value));
    }

    /**
     * Store a generic object under the provided key. The object will be wrapped into an McpStorageRecord.
     *
     * @param key   The key under which the value will be stored
     * @param value The object to store (wrapped in McpStorageRecord)
     */
    public static void write(String key, Object value) {
        getMap().put(key, McpStorageRecord.of(null, key, value));
    }

    /**
     * Read the stored record for the given key.
     * <p>
     * Note: the map stores McpStorageRecord instances. The method signature is generic to allow
     ...
     * @return The stored value (typically an McpStorageRecord) associated with the key, or null if not found
     */
     ...
     * <p>
     * If the key is not present, the mappingFunction is invoked and its result is stored using
     * the provided clazz identifier and key.
     *
     * @param clazz           Identifier for the schema/class associated with the value; may be used by consumers
     ...
     * <p>
     * - In shared/global mode, the single global storage map is cleared.
     * - In thread-local mode, the ThreadLocal map for the current thread is removed.
     * <p>
     * This method is also registered as a JVM shutdown hook to ensure cleanup at process exit.
     */
     ...
     */
    public static HashMap<String, McpStorageRecord> getCopy() {
        return new HashMap<>(getMap());
    }

    private static HashMap<String, McpStorageRecord> getMap() {
        if (sharedBetweenThreads.get()) return storage;

        HashMap<String, McpStorageRecord> map = threadLocalData.get();
        if (map == null) {
            map = new HashMap<>();
