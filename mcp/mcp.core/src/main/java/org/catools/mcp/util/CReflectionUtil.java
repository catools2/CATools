...
/**
 * Utility providing reflection-based method caching and invocation capabilities for MCP server
 * components. Implements singleton pattern to maintain a centralized method metadata cache across
 * the application.
 *
 * <p>This enum-based singleton ensures thread-safe method caching and provides optimized reflection
 * operations by avoiding repeated method metadata extraction.
 */
@Slf4j
public enum CReflectionUtil {
  ...
  /**
   * Retrieves cached method metadata or creates and caches it if not present. Uses
   * compute-if-absent pattern for thread-safe lazy initialization.
   *
  ...
  /**
   * Invokes a method using cached metadata with the provided parameters. Handles both successful
   * invocations and exceptions, returning a structured result.
   *
  ...
  /**
   * Invokes a parameterless method using cached metadata. Convenience method for zero-argument
   * method invocations.
   *
   * @param instance the object instance on which to invoke the method
   * @param methodCache cached method metadata containing invocation details
