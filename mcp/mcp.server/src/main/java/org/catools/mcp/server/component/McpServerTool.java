...
import org.catools.mcp.annotation.McpJsonSchemaDefinition;
 ...
 */
@Slf4j
public class McpServerTool implements McpServerComponent<McpServerFeatures.SyncToolSpecification> {
    ...
    public McpServerTool() {
        this.parameterConverter = getInjector().getInstance(McpToolParameterConverter.class);
    }
    ...
    public McpServerFeatures.SyncToolSpecification create(Method method) {
        ...
        final String name = StringUtils.defaultIfBlank(toolMethod.name(), methodCache.getMethodName());
        ...
        log.debug("Registering tool: {}", JsonUtil.toString(tool));
    ...
    private Map<String, Object> createJsonSchemaDefinition(Class<?> definitionClass) {
        ...
        for (Field field : fields) {
            ...
            final String fieldName = StringUtils.defaultIfBlank(property.name(), field.getName());
