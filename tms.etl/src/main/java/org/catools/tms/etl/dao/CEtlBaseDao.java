...
public class CEtlBaseDao {
  ...
  public static <T> void persist(CCollection<T> records) {
    log.trace("Performing bulk persist on {} records.", records.size());
    bulk(records, (session, record) -> session.persist(record));
  }
  ...
  public static <T> void merge(CCollection<T> records) {
    log.trace("Performing bulk merge on {} records.", records.size());
    bulk(records, (session, record) -> session.merge(record));
  }

  public static <T> void remove(CCollection<T> records) {
    log.trace("Performing bulk remove on {} records.", records.size());
    bulk(records, (session, record) -> session.remove(session.merge(record)));
  }

  protected static <T> void bulk(CCollection<T> records, BiConsumer<EntityManager, T> action) {
    int partitionSize = CEtlConfigs.getEtlBulkTransactionPartitionSize();
    records.partition(partitionSize).stream().forEach(partition -> {
      doTransaction(
          session -> {
