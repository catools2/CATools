...
public class COMDBAClientTest extends CTest {
  ...
  /**
   * Base method to verify search end point functionality. I moved method to the base class so I can
   * use it for different test implementation I have
   *
  ...
  public void verifySearchMethodResponseContainsExpectedTitles(
      String search, String[] expectedTitles) {
    verifySearchMovieTitleInResponse(search, expectedTitles);
  }

  @DataProvider(name = "TotalResult")
  public Object[][] dpTotalResult() {
    return new Object[][] {{"stem", 30}};
  }
  ...
  public void verifyGetByIdReturnsExpectedRecord(
      String search, String title, String released, String director) {
    Movies response =
        new OmdbSearchApiClient(Map.of(SearchParam.SEARCH, search))
            .readAllPages()
            .getByTitle(title);
