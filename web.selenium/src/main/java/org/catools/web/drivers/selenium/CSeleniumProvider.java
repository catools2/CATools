 ...
 *
 * <p>This interface defines the contract for creating WebDriver instances that can be configured to
 * run locally, remotely, or in test containers. It supports different browsers and provides
...
public interface CSeleniumProvider extends CDriverEngineProvider {
   ...
   *
   * <p>This method creates a WebDriver instance based on the current configuration:
   *
   * <ul>
   *   <li>Remote driver if {@code isUseRemoteDriver()} returns true
   *   <li>Test container if {@code isUseTestContainer()} returns true
   *   <li>Local driver otherwise
   * </ul>
   *
   * <p>The method also handles listener lifecycle callbacks:
   *
   * <ul>
   *   <li>{@code beforeInit()} is called before driver creation
   *   <li>{@code afterInit()} is called after driver creation
   * </ul>
   *
   * <h3>Examples:</h3>
   *
  ...
  default CDriverEngine build() {
    ...
    if (isUseRemoteDriver()) webDriver = buildRemoteWebDrier();
    else if (isUseTestContainer()) webDriver = buildTestContainer();
    else webDriver = buildLocalDriver();
   ...
   *
   * <p>This method creates a WebDriver that connects to a browser running inside a Docker container
   * or similar containerized environment. This is useful for consistent testing environments and
   * CI/CD pipelines.
   *
   * <h3>Examples:</h3>
   *
   ...
   *
   * <p>This method creates a WebDriver that launches and controls a browser process directly on the
   * local machine. The browser executable must be available in the system PATH or explicitly
   * configured.
   *
   * <h3>Examples:</h3>
   *
   ...
   *
   * <p>This method creates a WebDriver that connects to a remote Selenium server or grid, allowing
   * tests to run on different machines or cloud services. The remote endpoint configuration is
   * typically provided through system properties or configuration files.
   *
   * <h3>Examples:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>This method returns the specific browser enum value that identifies which browser this
   * provider is designed to work with. This is useful for determining provider capabilities and for
