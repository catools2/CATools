...
import org.catools.web.config.selenium.CEdgeConfigs;
...
/**
 * Microsoft Edge WebDriver provider implementation that creates and configures Edge WebDriver
 * instances.
 *
 * <p>This class provides functionality to create Edge WebDriver instances in different modes:
 *
 * <ul>
 *   <li>Local Edge driver execution
 *   <li>Remote WebDriver execution via Selenium Grid
 *   <li>Test container execution for isolated testing
 * </ul>
 *
 ...
 * @since 1.0
 */
public class CEdgeSeleniumProvider implements CSeleniumProvider {
   ...
   *
   * <ul>
   *   <li>Sets the Edge binary path if configured via {@link CEdgeConfigs#getBinaryPath()}
   *   <li>Adds default arguments from {@link CEdgeConfigs#getDefaultArguments()}
   *   <li>Sets the page load strategy from {@link CEdgeConfigs#getPageLoadStrategy()}
   *   <li>Enables headless mode if configured via {@link CEdgeConfigs#isInHeadLessMode()}
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   *
   * <p>This method prepares and returns the {@link Capabilities} object that will be used to
   * configure the Edge WebDriver instance. The capabilities include all previously configured
   * options such as arguments, binary path, and page load strategy.
   *
   * <p>The method automatically enables downloads by calling {@code setEnableDownloads(true)} on
   * the Edge options before returning the capabilities.
   *
   * <h3>Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>This method creates an isolated Edge WebDriver instance using Testcontainers with the
   * official Selenium Edge standalone image. The container is automatically started and configured
   * with the capabilities from {@link #getCapabilities()}.
   *
   * <p>This approach is ideal for:
   *
   * <ul>
   *   <li>CI/CD environments where consistent browser versions are required
   *   <li>Parallel test execution with complete isolation
   *   <li>Testing without installing Edge browser locally
   *   <li>Ensuring reproducible test results across different environments
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   *
   * <p>This method creates a local Edge WebDriver instance using the configured options. The Edge
   * browser must be installed on the local machine, and the Edge driver binary should be available
   * in the system PATH or configured via WebDriverManager.
   *
   * <p>The method calls {@link #getCapabilities()} to ensure all configurations are applied before
   ...
   *
   * <p>This method creates a RemoteWebDriver instance that connects to a Selenium Grid hub to
   * execute tests on a remote Edge browser. The hub URL is obtained from the grid configuration via
   * {@link CGridConfigs#getHubURL()}.
   *
   * <p>This approach is suitable for:
   *
   * <ul>
   *   <li>Distributed test execution across multiple machines
   *   <li>Cross-platform testing without local browser installation
   *   <li>Scaling test execution in CI/CD environments
   *   <li>Running tests on different browser versions simultaneously
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   *
   * <p>This method returns {@link CBrowser#EDGE} to identify that this provider is specifically
   * designed for Microsoft Edge browser instances.
   *
   * <h3>Example:</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>The page load strategy determines when WebDriver considers a page to be loaded. This affects
   * the behavior of navigation methods like {@code driver.get()} and {@code
   * driver.navigate().to()}.
   *
   * <p>Available strategies:
   *
   * <ul>
   *   <li>{@link PageLoadStrategy#NORMAL} - Waits for all resources to load (default)
   *   <li>{@link PageLoadStrategy#EAGER} - Waits for initial HTML document to be loaded
   *   <li>{@link PageLoadStrategy#NONE} - Does not wait for any loading to complete
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   */
  public CEdgeSeleniumProvider setPageLoadStrategy(PageLoadStrategy pageLoadStrategy) {
   ...
   *
   * <p>This method allows you to customize the Edge browser behavior by adding command-line
   * arguments. Arguments are added to the existing set of arguments and will be applied when the
   * WebDriver instance is created.
   *
   * <p>Common Edge arguments include:
   *
   * <ul>
   *   <li>{@code --headless} - Run in headless mode (no GUI)
   *   <li>{@code --start-maximized} - Start with maximized window
   *   <li>{@code --disable-web-security} - Disable web security (for testing)
   *   <li>{@code --no-sandbox} - Disable sandbox (useful in containers)
   *   <li>{@code --disable-dev-shm-usage} - Disable /dev/shm usage (useful in containers)
   *   <li>{@code --remote-allow-origins=*} - Allow remote origins (for remote debugging)
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   */
  public CEdgeSeleniumProvider addArguments(Iterable<String> args) {
    for (String arg : args) {
   ...
   *
   * <p>This method allows you to specify a custom path to the Edge browser binary if it's not
   * installed in the standard location or if you want to use a specific version of Edge. This is
   * particularly useful when:
   *
   * <ul>
   *   <li>Testing with different Edge versions
   *   <li>Using portable Edge installations
   *   <li>Working in environments where Edge is not in the standard location
   *   <li>Using Edge Dev, Beta, or Canary versions
   * </ul>
   *
   * <h3>Example:</h3>
   *
   ...
   */
  public CEdgeSeleniumProvider setBinary(String path) {
