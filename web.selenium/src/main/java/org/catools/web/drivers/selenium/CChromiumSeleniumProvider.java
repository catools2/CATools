...
/**
 * Chrome WebDriver provider implementation that supports local, remote, and containerized Chrome
 * browser instances.
 *
 * <p>This provider offers comprehensive Chrome browser configuration options including headless
 * mode, mobile device emulation, plugin management, and custom Chrome binary paths. It supports
 * three execution modes:
 *
 * <ul>
 *   <li>Local driver execution
 *   <li>Remote WebDriver grid execution
 *   <li>Docker container-based execution using Testcontainers
 * </ul>
 *
...
public class CChromiumSeleniumProvider implements CSeleniumProvider {
   ...
   *
   * <p>This provider automatically applies aggressive performance optimizations for maximum test
   * speed:
   *
   * <ul>
   *   <li><strong>Images BLOCKED</strong> - profile.default_content_setting_values.images = 2
   *   <li><strong>Plugins disabled</strong> - Flash and other plugins blocked
   *   <li><strong>Popups disabled</strong> - No popup windows
   *   <li><strong>Geolocation disabled</strong> - No location requests
   *   <li><strong>Notifications disabled</strong> - No browser notifications
   *   <li><strong>Media stream disabled</strong> - No camera/microphone access
   *   <li><strong>Downloads disabled</strong> - No automatic downloads
   * </ul>
   *
   * <p><strong>Performance Impact:</strong>
   *
   * <ul>
   *   <li>Page load: 60-80% faster (images not downloaded)
   *   <li>Rendering: 30-50% faster (fewer resources to process)
   *   <li>Memory: 40-60% lower (no image caching)
   *   <li>Network: 70-90% less bandwidth (no image downloads)
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>Sets custom Chrome/Chromium binary path if configured
   *   <li>Applies default Chrome arguments from configuration
   *   <li>Enables/disables plugins based on configuration
   *   <li>Sets page load strategy from configuration (EAGER recommended)
   *   <li>Enables PDF opening in new tabs
   *   <li>Applies mobile device emulation if configured
   *   <li>Enables headless mode if configured
   * </ul>
   *
  ...
  public CChromiumSeleniumProvider() {
    ...
    if (CChromiumConfigs.isInHeadLessMode()) setHeadless();
  }

  private static ChromeDriverService buildDefaultService() {
    return new ChromeDriverService.Builder()
        .withLogLevel(ChromiumDriverLogLevel.fromLevel(CGridConfigs.getLogLevel()))
        .build();
  }
   ...
   *
   * <ul>
   *   <li>Custom preferences and plugin configurations
   *   <li>Download enablement
   *   <li>Insecure certificate acceptance
   *   <li>Chrome DevTools Protocol (CDP) support
   *   <li>BiDi protocol support for advanced automation
   * </ul>
   *
   ...
   * Selenium standalone Chrome image. The container is automatically started and configured with
   * the current provider's capabilities. This approach provides isolation and consistency across
   * different environments.
   *
   * <p><strong>Benefits of containerized execution:</strong>
   *
   * <ul>
   *   <li>Environment isolation and consistency
   *   <li>No need for local Chrome installation
   *   <li>Automatic cleanup when tests complete
   *   <li>Parallel execution support
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method creates a Chrome WebDriver that connects directly to a locally installed Chrome
   * browser. The driver uses the configured Chrome binary path (if specified) or the system's
   * default Chrome installation. A Chrome driver service is automatically configured with
   * appropriate logging levels.
   *
   * <p><strong>Prerequisites:</strong>
   *
   * <ul>
   *   <li>Chrome browser installed on the local machine
   *   <li>ChromeDriver executable available (handled by WebDriverManager if enabled)
   *   <li>Sufficient system permissions to launch Chrome processes
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   * @throws org.openqa.selenium.WebDriverException if Chrome cannot be launched or ChromeDriver is
   *     not found
   */
  @Override
   ...
   *
   * <p>This method creates a RemoteWebDriver that connects to a Selenium Grid hub URL configured
   * via {@link CGridConfigs}. The remote execution allows for distributed testing across multiple
   * machines and browsers, enabling scalable test execution.
   *
   * <p><strong>Configuration requirements:</strong>
   *
   * <ul>
   *   <li>Selenium Grid hub must be running and accessible
   *   <li>Chrome nodes must be registered with the hub
   *   <li>Hub URL must be properly configured in CGridConfigs
   *   <li>Network connectivity between client and hub
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   * @throws org.openqa.selenium.WebDriverException if connection to hub fails or no Chrome nodes
   *     available
   */
  @Override
   ...
   *
   * <p>This method identifies the specific browser type that this provider manages, which is always
   * {@link CBrowser#CHROME} for this implementation. This information is used by the framework for
   * browser-specific configuration and routing.
   *
   * <p><strong>Example:</strong>
   *
   * <pre>{@code
   ...
   *
   * <p>This method allows specification of a custom Chrome browser binary instead of using the
   * system's default installation. This is useful for testing with different Chrome versions, beta
   * releases, or when Chrome is installed in a non-standard location.
   *
   * <p><strong>Common use cases:</strong>
   *
   * <ul>
   *   <li>Testing with Chrome Beta or Dev channels
   *   <li>Using portable Chrome installations
   *   <li>Custom Chrome builds with specific flags
   *   <li>Docker containers with Chrome in non-standard paths
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method allows adding custom Chrome command-line arguments to modify browser behavior,
   * enable experimental features, or configure security settings. Arguments are applied in addition
   * to any default arguments configured in {@link CChromiumConfigs}.
   *
   * <p><strong>Common arguments:</strong>
   *
   * <ul>
   *   <li>{@code --disable-web-security} - Disables web security (for testing)
   *   <li>{@code --no-sandbox} - Disables sandbox (required in some Docker environments)
   *   <li>{@code --disable-dev-shm-usage} - Prevents /dev/shm issues in containers
   *   <li>{@code --start-maximized} - Starts browser maximized
   *   <li>{@code --disable-notifications} - Disables browser notifications
   *   <li>{@code --disable-popup-blocking} - Disables popup blocking
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>Headless mode runs Chrome without a visible user interface, which is ideal for automated
   * testing, server environments, and CI/CD pipelines. The browser operates normally but without
   * rendering to a display, resulting in faster execution and lower resource consumption.
   *
   * <p>This method applies the headless arguments configured in {@link
   * CChromiumConfigs#getHeadLessArguments()}.
   *
   * <p><strong>Benefits of headless mode:</strong>
   *
   * <ul>
   *   <li>Faster test execution (no UI rendering)
   *   <li>Lower memory and CPU usage
   *   <li>Suitable for server environments without displays
   *   <li>Eliminates window focus issues
   *   <li>Enables parallel test execution
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This setting controls Chrome's behavior when encountering PDF files. When set to {@code
   * true}, PDFs will be displayed directly in the browser tab. When {@code false}, PDFs will be
   * downloaded to the default download directory. This is particularly useful for testing
   * applications that need to display or interact with PDF content.
   *
   * <p><strong>Use cases:</strong>
   *
   * <ul>
   *   <li>Testing PDF viewer functionality
   *   <li>Verifying PDF content without downloading
   *   <li>Preventing unwanted PDF downloads during tests
   *   <li>Enabling PDF interaction through WebDriver
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   * Different strategies offer trade-offs between speed and completeness of page loading. This
   * setting affects the behavior of {@code driver.get()} and navigation commands.
   *
   * <p><strong>Available strategies:</strong>
   *
   * <ul>
   *   <li>{@link PageLoadStrategy#NORMAL} - Wait for all resources to load (default)
   *   <li>{@link PageLoadStrategy#EAGER} - Wait for DOM to be ready, but not all resources
   *   <li>{@link PageLoadStrategy#NONE} - Return immediately after initial navigation
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method configures Chrome to emulate a mobile device, including viewport size, user
   * agent, touch events, and device pixel ratio. This enables testing mobile-responsive websites
   * and mobile-specific functionality without requiring actual mobile devices.
   *
   * <p><strong>Supported device names include:</strong>
   *
   * <ul>
   *   <li>iPhone 12, iPhone 12 Pro, iPhone SE
   *   <li>Samsung Galaxy S21, Samsung Galaxy Note 20
   *   <li>iPad, iPad Pro, iPad Mini
   *   <li>Pixel 5, Pixel 4, Pixel 3
   *   <li>Custom device configurations (see Chrome DevTools)
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method adds plugins to the disabled list, preventing them from loading during browser
   * startup. Disabling unnecessary plugins can improve browser performance, reduce security risks,
   * and eliminate potential interference with automated tests.
   *
   * <p><strong>Common plugins to disable:</strong>
   *
   * <ul>
   *   <li>Chrome PDF Viewer
   *   <li>Chrome Flash Player (legacy)
   *   <li>Chrome Default Apps
   *   <li>Chrome Translate
   *   <li>Chrome Password Manager
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method adds plugins to the enabled list, ensuring they are loaded and active during
   * browser startup. This is useful when you need specific plugin functionality for your tests, or
   * when overriding default disabled plugins from configuration.
   *
   * <p><strong>Example use cases:</strong>
   *
   * <ul>
   *   <li>Testing applications that require Flash (legacy)
   *   <li>Enabling PDF viewer for PDF interaction tests
   *   <li>Testing browser extension compatibility
   *   <li>Overriding globally disabled plugins for specific tests
   * </ul>
   *
   * <p><strong>Example:</strong>
   *
   ...
   *
   * <p>This method provides fine-grained control over individual plugin states. It's particularly
   * useful when you need to enable some plugins while disabling others, or when building plugin
   * configurations programmatically.
   *
   * <p>The plugin configuration is stored internally and applied when the browser capabilities are
   * built. Each plugin entry includes an enabled flag and the plugin name, allowing precise control
   * over Chrome's plugin loading behavior.
   *
   * <p><strong>Example:</strong>
   *
   * <pre>{@code
